...
function makeBeta({ effectful = false, extraUse = false }) {
    const lines = [
        "            local a = 1",
    ...
    const operations = [
        ...
        effectful
            ? { kind:"version-define", emittedTarget:"b", rhs:"side()", reads:["side"], emittedText:"local b = side()" }
            : { kind:"version-define", emittedTarget:"b", rhs:"nil", reads:[], emittedText:"local b = nil" },
...
}
{
    ...
    finalizePreCfLiteralReturnTemps(beta);
    assert.equal(beta.preCfLiteralReturnTemps.folds, 1);
