...
function normalizeRegisterOverflowGraphExperimental(graph) {
    const slots = new Set();
...
function regionGraph(graph, entry, ownerByState, solvedBodies) {
    ...
    const normalized = normalizeRegisterOverflowGraphExperimental({ ...graph, entries: [entry], states: rawStates });
...
function solveBetaControlFlow(originalAst, betaResult) {
    ...
    let solved;
    if (graph.entries.length === 1) {
        const normalized = normalizeRegisterOverflowGraphExperimental(graph);
...
module.exports = {
    ...
    normalizeRegisterOverflowGraphExperimental,
