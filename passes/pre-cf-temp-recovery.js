...
function applySourceEdits(source, edits) {
    ...
    for (const edit of sorted) {
        ...
        if (!Number.isInteger(start) || !Number.isInteger(end) || start < cursor || end < start || end > text.length) {
            throw new Error(`Invalid or overlapping PRE-CF source edit ${start}:${end}`);
        }
        parts.push(text.slice(cursor, start), String(edit.replacement ?? ""));
        cursor = end;
    }
...
function finalizePreCfCallResultDestinations(betaResult) {
    ...
    for (let round = 0; round < maxRounds; round++) {
        ...
        for (const facts of proof.byBinding.values()) {
            ...
            const touchedNames = new Set([facts.name, actualTarget, ...(producer.reads || []), ...operationWrites(producer), ...(consumer.reads || []), ...operationWrites(consumer)]);
            candidates.push({ facts, state, producer, consumer, producerOffset, consumerOffset, actualTarget, touchedNames });
        }
        if (!candidates.length) break;
        ...
        const ownership = mapPreCfOperationRanges(betaResult); parseRounds++;
        if (!ownership.safe) {
            betaResult.preCfCallResultDestinations = { applied: folds > 0, safe: false, reason: ownership.reason, folds, parseRounds, batchRounds };
            return betaResult;
        }
        const edits = [];
        for (const candidate of accepted) {
            ...
            if (!producerRange || !consumerRange) {
                betaResult.preCfCallResultDestinations = { applied: folds > 0, safe: false, reason: "PRE-CF call-result destination recovery lost exact source ownership", folds, parseRounds, batchRounds };
                return betaResult;
            }
            const prefix = String(candidate.consumer.emittedText || "").trim().startsWith("local ") ? "local " : "";
            candidate.emittedText = `${prefix}${candidate.actualTarget} = ${candidate.producer.rhs}`;
            edits.push(
                { start: producerRange[0], end: producerRange[1], replacement: candidate.emittedText },
        ...
        try { output = applySourceEdits(betaResult.source, edits); parsePreCfSource(output); parseRounds++; }
            ...
            betaResult.preCfCallResultDestinations = { applied: folds > 0, safe: false, reason: `PRE-CF call-result destination recovery reparse failed: ${error.message}`, folds, parseRounds, batchRounds };
            return betaResult;
        }
        betaResult.source = output;
        const removalsByState = new Map();
        for (const candidate of accepted) {
            ...
            candidate.producer.emittedText = candidate.emittedText;
            if (!removalsByState.has(candidate.facts.producer.stateId)) removalsByState.set(candidate.facts.producer.stateId, []);
            removalsByState.get(candidate.facts.producer.stateId).push(candidate.consumerOffset);
        }
        for (const [stateId, offsets] of removalsByState) {
            const state = proof.stateById.get(stateId);
    ...
    betaResult.preCfCallResultDestinations = { applied: folds > 0, safe: true, folds, parseRounds, batchRounds };
    return betaResult;
}
...
function finalizePreCfIndexKeyTemps(betaResult) {
    ...
    for (let round = 0; round < maxRounds; round++) {
        ...
        for (const facts of proof.byBinding.values()) {
            ...
            if (!producer || !consumer || claimedOperations.has(producer) || claimedOperations.has(consumer) || !isCopyOperation(producer)) continue;
            ...
            candidates.push({ facts, state, producer, consumer, producerOffset, rhs, emittedText });
        }
        if (!candidates.length) break;
        const ownership = mapPreCfOperationRanges(betaResult); parseRounds++;
        if (!ownership.safe) { betaResult.preCfIndexKeyTemps = { applied: folds > 0, safe: false, reason: ownership.reason, folds, parseRounds, batchRounds }; return betaResult; }
        const edits = [];
        for (const candidate of candidates) {
            const producerRange = ownership.ranges.get(candidate.producer);
            const consumerRange = ownership.ranges.get(candidate.consumer);
            if (!producerRange || !consumerRange) {
                betaResult.preCfIndexKeyTemps = { applied: folds > 0, safe: false, reason: "PRE-CF index key recovery lost exact source ownership", folds, parseRounds, batchRounds };
                return betaResult;
            }
            edits.push(
        ...
        try { output = applySourceEdits(betaResult.source, edits); parsePreCfSource(output); parseRounds++; }
            ...
            betaResult.preCfIndexKeyTemps = { applied: folds > 0, safe: false, reason: `PRE-CF index key recovery reparse failed: ${error.message}`, folds, parseRounds, batchRounds };
            return betaResult;
        }
        betaResult.source = output;
        const removalsByState = new Map();
        for (const candidate of candidates) {
            ...
            if (!removalsByState.has(candidate.state.id)) removalsByState.set(candidate.state.id, new Set());
            removalsByState.get(candidate.state.id).add(candidate.producer);
        }
        for (const [stateId, removals] of removalsByState) {
            const state = proof.stateById.get(stateId);
            state.operations = state.operations.filter(operation => !removals.has(operation));
    ...
    betaResult.preCfIndexKeyTemps = { applied: folds > 0, safe: true, folds, parseRounds, batchRounds };
    return betaResult;
}
...
function finalizePreCfIndexedWriteTemps(betaResult) {
    ...
    let folds = 0, keyTempsRemoved = 0, rhsTempsRemoved = 0, parseRounds = 0, batchRounds = 0;
    const maxRounds = (betaResult.graph.states || []).reduce((n, state) => n + (state.operations || []).length, 0) + 1;
    for (let round = 0; round < maxRounds; round++) {
        const proof = buildPreCfTempProofIndex(betaResult);
        const candidates = [];
        ...
        for (const state of betaResult.graph.states || []) {
            ...
            for (let offset = 0; offset < operations.length; offset++) {
                ...
                if (!info || claimedOperations.has(operation)) continue;
                const replacements = [];
                const producers = new Set();
                for (const slot of [
                    ...
                    const producer = facts.producer.operation;
                    if (claimedOperations.has(producer) || producers.has(producer)) continue;
                    if (!isCopyOperation(producer) || !String(producer.emittedText || "").trim().startsWith("local ")) continue;
                    const expression = parsePreCfRhs(producer.rhs);
                    if (!isLiteralOnlyPreCfScalarExpression(expression)) continue;
                    let blocked = false;
                    for (let i = facts.producer.offset + 1; i < offset; i++) {
                        const gap = operations[i];
                        if ((gap.reads || []).includes(slot.binding) || operationWrites(gap).includes(slot.binding)) { blocked = true; break; }
                    }
                    if (blocked) continue;
                    replacements.push({ ...slot, facts, producer, producerOffset: facts.producer.offset, replacement: producer.rhs });
                    producers.add(producer);
                }
                if (!replacements.length) continue;
                let emittedText = String(operation.emittedText || "");
                for (const replacement of [...replacements].sort((x, y) => y.start - x.start)) {
                    emittedText = emittedText.slice(0, replacement.start) + replacement.replacement + emittedText.slice(replacement.end);
                }
                candidates.push({ state, operation, offset, replacements, producers, emittedText });
        ...
        if (!candidates.length) break;
        const ownership = mapPreCfOperationRanges(betaResult); parseRounds++;
        if (!ownership.safe) { betaResult.preCfIndexedWriteTemps = { applied: folds > 0, safe: false, reason: ownership.reason, folds, keyTempsRemoved, rhsTempsRemoved, parseRounds, batchRounds }; return betaResult; }
        const edits = [];
        for (const candidate of candidates) {
            const consumerRange = ownership.ranges.get(candidate.operation);
            const producerRanges = [...candidate.producers].map(producer => ownership.ranges.get(producer));
            if (!consumerRange || producerRanges.some(range => !range)) {
                betaResult.preCfIndexedWriteTemps = { applied: folds > 0, safe: false, reason: "PRE-CF indexed write recovery lost exact source ownership", folds, keyTempsRemoved, rhsTempsRemoved, parseRounds, batchRounds };
                return betaResult;
            }
            edits.push({ start: consumerRange[0], end: consumerRange[1], replacement: candidate.emittedText });
            for (const range of producerRanges) edits.push({ start: range[0], end: range[1], replacement: "" });
        }
        let output;
        try { output = applySourceEdits(betaResult.source, edits); parsePreCfSource(output); parseRounds++; }
        catch (error) {
            betaResult.preCfIndexedWriteTemps = { applied: folds > 0, safe: false, reason: `PRE-CF indexed write recovery reparse failed: ${error.message}`, folds, keyTempsRemoved, rhsTempsRemoved, parseRounds, batchRounds };
            return betaResult;
        }
        betaResult.source = output;
        const producersByState = new Map();
        for (const candidate of candidates) {
            ...
            const removedBindings = new Set(candidate.replacements.map(item => item.binding));
            candidate.operation.reads = (candidate.operation.reads || []).filter(name => !removedBindings.has(name));
            if (!producersByState.has(candidate.state.id)) producersByState.set(candidate.state.id, new Set());
            for (const producer of candidate.producers) producersByState.get(candidate.state.id).add(producer);
            for (const replacement of candidate.replacements) {
                if (replacement.kind === "key") keyTempsRemoved++;
        ...
        for (const [stateId, producers] of producersByState) {
            const state = proof.stateById.get(stateId);
            state.operations = state.operations.filter(operation => !producers.has(operation));
            for (let i = 0; i < state.operations.length; i++) state.operations[i].index = i + 1;
        }
        batchRounds++;
    }
    betaResult.preCfIndexedWriteTemps = { applied: folds > 0, safe: true, folds, keyTempsRemoved, rhsTempsRemoved, parseRounds, batchRounds };
    return betaResult;
}
...
function finalizePreCfCallArgumentTemps(betaResult) {
    ...
    for (let round = 0; round < maxRounds; round++) {
        ...
        const candidates = [];
        const claimedOperations = new Set();
        const parsedRhs = new Map();
        const parseRhsCached = operation => {
            if (!parsedRhs.has(operation)) parsedRhs.set(operation, parsePreCfRhs(operation?.rhs));
            return parsedRhs.get(operation);
        };
        ...
        for (const facts of proof.byBinding.values()) {
            ...
            const producer = facts.producer.operation;
            const consumer = facts.consumer.operation;
            if (claimedOperations.has(producer) || claimedOperations.has(consumer)) continue;
            if (!isCopyOperation(producer) || !isCopyOperation(consumer)) continue;
            const producerExpr = parseRhsCached(producer);
            ...
            candidates.push({ state: proof.stateById.get(facts.producer.stateId), consumer, producerItems: [{ facts, producer }], rewritten });
            claimedOperations.add(producer);
            claimedOperations.add(consumer);
        }
        ...
        for (const state of betaResult.graph.states || []) {
            ...
            for (let consumerOffset = 0; consumerOffset < operations.length; consumerOffset++) {
                ...
                if (claimedOperations.has(consumer) || !isCopyOperation(consumer)) continue;
                const consumerExpr = parseRhsCached(consumer);
                ...
                for (let producerOffset = consumerOffset - 1; producerOffset >= 0; producerOffset--) {
                    ...
                    if (claimedOperations.has(producer)) { conflict = true; break; }
                    const producerExpr = parseRhsCached(producer);
                ...
                if (conflict || !producerItems.length) continue;
                const hasLookupProducer = producerItems.some(item => isStaticLookupExpression(parseRhsCached(item.producer)));
                ...
                candidates.push({ state, consumer, producerItems, rewritten });
                claimedOperations.add(consumer);
                for (const item of producerItems) claimedOperations.add(item.producer);
            }
        }
        ...
        const ownership = mapPreCfOperationRanges(betaResult); parseRounds++;
        if (!ownership.safe) {
            betaResult.preCfCallArgumentTemps = { applied: folds > 0, safe: false, reason: ownership.reason, folds, parseRounds, batchRounds };
            return betaResult;
        }
        const edits = [];
        for (const candidate of candidates) {
            ...
            const consumerRange = ownership.ranges.get(candidate.consumer);
            if (producerRanges.some(range => !range) || !consumerRange) {
                betaResult.preCfCallArgumentTemps = { applied: folds > 0, safe: false, reason: "PRE-CF call-argument recovery lost exact source ownership", folds, parseRounds, batchRounds };
                return betaResult;
            }
            const prefix = String(candidate.consumer.emittedText || "").trim().startsWith("local ") ? "local " : "";
            candidate.emittedText = `${prefix}${candidate.consumer.emittedTarget} = ${candidate.rewritten.rhs}`;
        ...
        try { output = applySourceEdits(betaResult.source, edits); parsePreCfSource(output); parseRounds++; }
            ...
            betaResult.preCfCallArgumentTemps = { applied: folds > 0, safe: false, reason: `PRE-CF call-argument recovery reparse failed: ${error.message}`, folds, parseRounds, batchRounds };
            return betaResult;
        }
        betaResult.source = output;
        const removalsByState = new Map();
    ...
    betaResult.preCfCallArgumentTemps = { applied: folds > 0, safe: true, folds, parseRounds, batchRounds };
    return betaResult;
}
