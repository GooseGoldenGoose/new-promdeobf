...
Step 12 checkpoint: CF acyclic branch structuring now consumes an exact adjacent single-use condition beta temp into the emitted if header. The producer must be the final body operation before the proven branch transition, be a normal beta scalar definition, have exactly one graph read, and not be a recovered upvalue binding. Its RHS stays at the same evaluation point and is parenthesized; intervening statements, captures, and extra uses fail closed. Focused test: tools/test-beta-cf-condition-temps.js covers direct calls, and, not, indexed comparisons, and refusal edge cases. Full beta-CF suite passes and canonical numeric samples 1-66 pass 66/66.
