...
Latest beta/CF behavior checkpoint is `5f7c7f4 Recover numeric for loops with branched bodies` on `main`, pushed to `origin/main`; latest fixture checkpoint is `40b41fa Add nested numeric for sample 26`. Beta-CF structurally recognizes the local WeAreDevs compiler numeric `ForStatement` lowering and allows the loop body to be an acyclic internal CFG region with branches/joins and already-structured nested numeric-for nodes before proven backedge latches. Iterative innermost-first collapse therefore supports sequential and nested numeric loops while still failing closed on unrelated cycles or escaping body edges. Terminal-return and captured-binding protections remain unchanged. All tracked fixtures `sample/1` through `sample/26` remain paired.
...
- Latest pushed behavior checkpoint at this snapshot: `5f7c7f4 Recover numeric for loops with branched bodies`; latest fixture checkpoint is `40b41fa Add nested numeric for sample 26`; earlier numeric-for checkpoint is `08b53ca Recover Prometheus numeric for loops`. Beta-CF supports compiler-proven numeric `for` loops while unrelated/general cycles still fail closed. Terminal compiler return payloads, nested-child return safety, captured-cell lexical ordering, ambiguous assignments, unresolved captures/cells, and multi-state cell-owner safeguards remain enforced.
- Indexed/member effect-write support from `f16b244` remains active and is consumed normally by beta upvalue recovery/CF.
- Latest fixture checkpoint is `40b41fa Add nested numeric for sample 26`; latest numeric-for CF checkpoint is `5f7c7f4 Recover numeric for loops with branched bodies`; tracked samples 1-26 remain paired.
