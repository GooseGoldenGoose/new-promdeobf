...
Dead state/POS initializer verification: the stronger transitive definition proof eliminates all residual `= state` saves in the canonical pre-CF `spacial6` path. With overflow scalarization first, `spacial6` has 220 overflow slots, 0 `RegisterOverflow[...]` refs, 507 dead snapshot operations removed (252 roots / 255 copies), then 10 dead state definitions removed/cleared (including 2 assignment-form saves), 33 dead propagated copies removed, and 2 reused-local copy initializers cleared; final pre-CF `= state` count is 0. Dedicated regressions cover transitive copy chains, overwritten copied definitions, assignment-form saves, and a real `state = savedCopy` restore that must stay live. Pre-CF-focused suites pass 12/12. Numeric fixtures 1-63 all pass the full canonical pipeline through dead-state initializer cleanup with CF intentionally not run; aggregate output has 0 `RegisterOverflow[...]` refs and 0 `= state` survivors.
