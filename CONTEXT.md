...
- Latest frozen complete batch: 27/50 recovered and independently Luau-compiled; report `output/production-2026-09-08T01-19-36-292Z/results.json`. A later mixed-revision batch reached 29/50 with one timeout (`output/production-2026-09-08T04-28-54-630Z/results.json`), but it is diagnostic only because recovery code changed while it ran. Current code contains further fixes and needs a new frozen all-50 batch. These counts are progress measurements, not correctness certification.
...
- Current fixes: structured self-recursive cell binding; preserve unused cleanup-backed and captured multi-return locals; recognize terminal return before dead nil cleanup; avoid exponential revisiting in loop-storage/read-before-write, cleanup, index-write, and later-read graph queries; require live join use for new direct logical-result candidates so ordinary conditionals are not erased; recognize source tables with fixed fields followed by a proven return-pack expansion.
...
- Additional fixes: terminal generic-for bodies with only breaks/returns, including return paths without nil cleanup; generic-for ambiguity checking limited to the compiler's contiguous nil prefix; newly allocated captured locals initialized from a multi-return call (bind the exact cell identity to the already emitted local); carry a proven call return-pack into later blocks when read before overwrite; forward nested packs; separate parenthesized call statements from preceding expressions; materialize complex deferred expressions read at multiple later sites so calls/tables are evaluated once; close their mutation-base ownership correctly; discard compiler-only dead temporary cleanup inside a proven loop without inventing a source local.
- The generic-for correction recovered `0003d9e06049f5ed.lua` (210 states, 24 closures). Pack and graph-query fixes advanced `0013460882ac1c6b.lua` through its 18 nested/manual `string.gmatch` loops and removed a greater-than-90-second merge hang; the same point now processes in about one second.
- Before the latest late-stage solver/lifetime edits, all 19 established suites passed. The deterministic randomized compiler gate passed 36/36 exact behavior checks across six fixture categories and six distinct seeds (`output/production-fixtures-1788830441081/`). After the latest edits, the focused Fresh, while, and production-recovery suites pass; the full 19-suite gate and 36-case randomized gate must be rerun before certification. `fast-obf-deobf.js --seed <positive integer>` supports reproducible compiler layouts.
...
- Current active failure: `0013460882ac1c6b.lua`, child entry 205, state 235, source statement `r18 = releaseUpvalue(r18)`. The final structured-solver guard rejects because the graph finishes with unreleased local/cell bookkeeping (`ctx.locals.size !== 0`), not because the state is unprocessed. Diagnose the exact cell identity and release lifetime; fix from compiler/dataflow proof, never from this register or filename.
- Next: finish that general release-upvalue fix; rerun the focused sample and independent Luau compilation; run a new frozen all-50 report; group remaining failures; rerun all 19 established suites, `tools/test-beta-production-recovery.js`, and the 36-case deterministic behavior gate. Completion still requires 50/50 recovery, 50/50 Luau syntax/compile, no retained VM/placeholders/unresolved regions, and all regression/behavior gates passing.
