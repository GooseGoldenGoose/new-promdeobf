...
Latest beta behavior checkpoint is `61bd249 Canonicalize beta terminal return placement` on `main`, pushed to `origin/main`. Beta register versioning now canonicalizes each proven terminal return pair before any control-flow structuring: `ReturnVal = { ... }` is placed immediately above the final `state = nil`, and that stop must already be the last normalized state statement. Empty payloads may move across leftover tail work because they carry no returned-value dependency; non-empty payloads move only across structurally pure bookkeeping and never across an effectful operation or a write to a returned version. The one-state beta CF stage can then lower the adjacent pair to real Lua `return ...`. Multi-state CFG structuring is still not implemented. Beta remains analysis/presentation-only. All tracked fixtures `sample/1` through `sample/14` have matching `.source.lua` + formatter-normalized `.txt` pairs. Source lexical-scope reconstruction has NOT been implemented yet.
...
- Beta control-flow presentation lives in `passes/beta-control-flow.js` with CLI `node tools/beta-control-flow.js <output.lua> [output.beta.cf.lua]`. The current stage accepts only a closed CFG with exactly one entry, one state, and a stop terminator; it emits `--headers`, a structurally derived `_env` binding when needed, `local args = { ... }`, and a flat `--body`. A terminal compiler-shaped `ReturnVal` table is converted into source-level return syntax: `{}` -> `return`, `{r1, r2}` -> `return r1, r2`, and a final `{r1, unpack(r2)}` -> `return r1, unpack(r2)`. Multi-state graphs, unsupported operations, unknown environment providers, and non-terminal entries still fail closed.
...
- Latest pushed beta behavior checkpoint at this snapshot: `61bd249 Canonicalize beta terminal return placement`; beta register versioning now canonicalizes proven terminal `ReturnVal` placement immediately above the final `state = nil`, and one-state beta CF lowers that canonical pair to Lua `return` syntax.
