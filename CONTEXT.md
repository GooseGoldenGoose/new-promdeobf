...
Step 10 checkpoint: added `finalizePreCfMultiReturnTemps`. It recognizes only the exact same-state compiler pack `local pack = { call(...) }` followed immediately by contiguous local static slot extractions `pack[1..N]` with no other pack use/capture, then replaces that region with one native `local a, b, ... = call(...)` and one `multi-call-write` graph operation. Sparse/out-of-order slots, pack escape, nonlocal extraction targets, and non-single-call tables fail closed. Focused test: `tools/test-pre-cf-multi-return-temps.js`.
...
Multi-return slot-order recovery checkpoint (2026-08-29): `finalizePreCfMultiReturnTemps` now accepts the local compiler's contiguous static extraction block even when extraction statements are emitted out of slot order. It requires unique slots forming exactly `1..N`, proves all pack reads are exactly those extractions, then sorts targets/epochs by slot number before emitting one native `multi-call-write`. Duplicate/sparse slots, pack escape, captures, nonlocal extracts, and non-single-call packs still fail closed. Exact Medium probe `tmp/pcall-mr-418818ce695d4c9cbdd154ab928570cb/final.step2.lua:13` is now `local r_v4_2, r_v6_1 = r_v2_1(r_v4_1, r_v3_2)` with no `{ call(...) }` pack or `[1]/[2]` extraction scaffolding. Focused multi-return suite PASS; combined gate 17 focused suites + 66/66 canonical PASS; Luau source/obfuscated/final all exit 0 with matching `true + table:` result shape.
