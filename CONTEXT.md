...
Latest beta behavior checkpoint is `757018e Rewrite beta reads in indexed assignments` on `main`, pushed to `origin/main`. Beta register versioning now rewrites proven reaching beta versions inside read positions of assignment shapes whose writes remain unsupported, including complex/indexed LHS base/index expressions and every RHS expression; plain identifier LHS targets remain writes and are never rewritten as reads. This fixes captured-cell writes such as `upvalueValues[r1] = state` without pretending the indexed store itself is a scalar beta definition. Beta CF still supports the existing closed acyclic single-entry and empty-capture multi-closure regions; captured-closure reconstruction itself remains fail-closed. All tracked fixtures `sample/1` through `sample/18` remain paired.
...
- Latest pushed beta behavior checkpoint at this snapshot: `757018e Rewrite beta reads in indexed assignments`; beta now preserves reaching beta values inside indexed/complex assignment reads while keeping unsupported writes conservative. Existing acyclic control-flow and empty-capture closure-region support remains unchanged; captured-closure reconstruction and loops/backedges still fail closed.
...
- Sample 18: paired captured-mutable-upvalue fixture from `local a = 123; local b = function() a = a + 1; return true end; print(b()); print(a)`. It follows readable source -> local WeAreDevs Medium -> formatter. Main resolves 2/2 states across root `createClosure` state 1 and child `createClosure5` state 2. Step-4 capture analysis proves 1 capture slot, 1 local upvalue cell, 4/4 upvalue cell accesses resolved, and 1 captured binding candidate. Beta still reports 2 skipped assignments because the two `upvalueValues[...] = ...` stores are unsupported indexed write shapes, but `757018e` now rewrites their read positions structurally: root initialization becomes `upvalueValues[r_v1_1] = r_v2_1` and the child mutation becomes `upvalueValues[upvalues[1]] = r_v2_5`. The write shapes remain conservative/non-versioned definitions while graph metadata records the beta-version reads. Readable source, formatted obfuscation, normal deobfuscation, and `output/18.beta.lua` now all execute identically and print `true` then `124`. Focused beta regression covers the generic indexed-assignment read rewrite; beta register graph and beta control-flow regression suites remain green.
