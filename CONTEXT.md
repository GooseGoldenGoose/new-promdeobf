...
Latest implementation checkpoint is `46f3973 Recover structured loop control edges` on `main`, pushed to `origin/main`; latest fixture checkpoint is `4cdc9c3 Add minimal loop blocker samples 45-46`. Beta-CF supports compiler-proven numeric `for`, pre-test `while`, post-test `repeat ... until`, explicit `break`, proven `continue` edges, early `return` inside loop bodies, and multi-state short-circuit loop conditions. Control-only joins are structured directly rather than introducing `_beta_phi` helpers. Repeat recovery still removes the local compiler's discarded first condition evaluation to restore readable-source semantics. All tracked fixtures `sample/1` through `sample/46` remain paired.
...
5. Remaining focused beta-CF blockers from samples 31-44 are now sample 35 (multi-state captured-cell lexical dominance inside loops) and sample 37 (source mutation of a numeric-for loop variable). Break/continue/early-return and multi-state short-circuit loop conditions are implemented and covered by samples 31-34, 36, 38-44. Keep unsupported capture/mutation shapes fail-closed; generic iterator `for ... in` remains intentionally untested per user request.
...
- Latest pushed implementation checkpoint at this snapshot: `46f3973 Recover structured loop control edges`; latest normal-pipeline optimization is `11a5629 Optimize deobfuscation pipeline passes`; latest fixture checkpoint is `66cfa59 Add loop control stress samples 39-44`. Beta-CF supports numeric `for`, pre-test `while`, post-test `repeat`, loop `break`, proven `continue`, terminal `return` arms inside loops, and multi-state short-circuit conditions. Native `continue` is emitted for proven for/while latch paths; repeat-body continue paths use safe fallthrough when condition setup statements must execute before `until`. Control-only short-circuit joins are structured without `_beta_phi` helper locals. Repeat still removes the compiler-added discarded first condition evaluation to restore source semantics. Multi-state captured-cell dominance and numeric-for source-variable mutation remain fail-closed.
- Indexed/member effect-write support from `f16b244` remains active and is consumed normally by beta upvalue recovery/CF.
- Latest fixture checkpoint is `66cfa59 Add loop control stress samples 39-44`; latest loop-control implementation is `46f3973 Recover structured loop control edges`; tracked samples 1-46 remain paired.
...
- Samples 31-44 are the focused loop-control matrix (generic iterator `for ... in` intentionally excluded by user request). All paired sources compile through local WeAreDevs Medium + formatter; normal deobfuscation and beta register versioning succeed with 0 skipped assignments.
  ...
  - Sample 35: still fail-closed at beta upvalue recovery because captured cells are allocated in multi-state owner regions and lexical declaration dominance/hoisting proof is not implemented. Exact first blocker: root numeric-for body state 3 allocates `r_v8_4 = allocUpvalue()`, initializes `upvalueValues[r_v8_4]` from the loop value, creates child entry 11 with that cell, then releases it before the backedge. `passes/beta-upvalues.js` currently requires `regionStateCount[cell.ownerEntry] === 1`, so the multi-state root owner fails before CF structuring. This is now one of the two remaining loop-matrix blockers.
  ...
  - Sample 37: still fail-closed intentionally because source mutation of the numeric-for loop variable is not yet distinguished from compiler induction/loop-variable bookkeeping strongly enough for safe recovery. Exact body state 3 writes the recovered loop binding three ways: compiler copy `local r_v8_4 = r_v3_3`, source mutation `r_v8_4 = r_v2_4` after computing `r_v2_4 = r_v8_4 + 10`, and compiler cleanup `r_v8_4 = nil`. `matchCompilerNumericFor` rejects any loop-variable write other than the one compiler definition plus proven cleanup (`loopVariableWrites` guard), leaving the backedge unresolved. This is the second remaining loop-matrix blocker.
