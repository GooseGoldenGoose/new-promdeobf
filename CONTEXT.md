...
Latest beta behavior checkpoint is `b4ad0c8 Fix beta numeric loop register flow` on `main`, pushed to `origin/main`; latest scheduler checkpoint is `ecc6075 Move numeric VM transitions to leaf tail`, with earlier capture/terminal-return checkpoints `d919b11 Hoist captured beta bindings` and `9fbb616 Canonicalize terminal VM returns`. The scheduler now identifies the real VM return register structurally from the VM container's final `return unpack(<register>)` and, inside a Step-3-proven terminal leaf, canonicalizes a compiler-shaped return payload to sit immediately before final `state = nil`. It crosses only proven compiler bookkeeping (fixed `args[n]` loads, recognized `upvalueValues[...]` initialization, and non-terminating POS/state temporary writes) and never crosses effectful calls or writes to a returned value. Beta upvalue recovery also handles captured cells whose closure is created before first initialization by hoisting only the recovered lexical declaration to the original allocation point and leaving the value assignment at the original initialization point, preserving Lua capture scope. These changes remove the former sample-20 and sample-23 nested-return blockers without weakening the fail-closed child-return guard. All tracked fixtures `sample/1` through `sample/24` remain paired.
...
- Every tracked `sample/N.txt` must have a matching tracked `sample/N.source.lua`. Current paired coverage is samples 1-24.
...
- Current paired fixture coverage is samples 1-24. New sample 5 schedules 390 blocks with 221,956 dependency-safe swaps, 217 unread sinks, and a structurally proven 23-slot overflow bank. Sample 10 remains the natural overflow-boundary fixture with 19 slots.
...
- Focused regression `tools/test-semantic-names.js` covers `InitialArgs`, fixed closure `argN`, `gcProxy`, `closure`, `captureIndex`, `upvalueId`, `proxy`, `proxyMetatable`, and `RegisterOverflow`. Current paired fixtures 1-23 remain parse/runtime-valid where executable.
...
5. Extend beta CFG structuring to loops/backedges using dominators/backedge loop discovery before emitting `while`/`repeat`; sample 4 remains the focused if/while/repeat regression, sample 6 the nested-function/if/while regression, and sample 7 the branch/early-return + capture regression.
...
- Latest pushed behavior checkpoint at this snapshot: `ecc6075 Move numeric VM transitions to leaf tail`; latest beta checkpoint is `b4ad0c8 Fix beta numeric loop register flow`, with preceding capture/terminal-return checkpoints `d919b11 Hoist captured beta bindings` and `9fbb616 Canonicalize terminal VM returns`. Terminal compiler return payloads are canonicalized immediately before a Step-3-proven final `state = nil` across only recognized bookkeeping, and captured bindings are declared before any child closure that must lexically capture them even when the compiler initializes the cell later. The non-root fully-lowered-return guard remains enforced. Ambiguous assignments, unresolved captures/cells, multi-state cell owners, unlowered child returns, and loops/backedges still fail closed.
- Indexed/member effect-write support from `f16b244` remains active and is consumed normally by beta upvalue recovery/CF.
- Latest fixture checkpoint is `9474c28 Add numeric for-loop sample 24`; latest beta behavior checkpoint is `b4ad0c8 Fix beta numeric loop register flow`; latest scheduler checkpoint is `ecc6075 Move numeric VM transitions to leaf tail`; tracked samples 1-24 remain paired.
...
- Sample 24: paired numeric `for` regression from `for i = 1,10 do print(i) end`. It follows readable source -> local WeAreDevs Medium -> formatter. Normal deobfuscation resolves 4/4 states in one VM function, with 29 reaching definitions, 1 join, 5 cross-block lifetimes, and 1 loop-carried lifetime; source/raw/formatted/normal all execute identically and print `1` through `10`. Beta register versioning now reuses Step-3 `analyzeBlockTerminator` for CFG successors, so the compiler split `state = cond and temp; ...; state = state or temp` numeric-for branch is recognized instead of marking the beta CFG incomplete. Current beta reports 4 blocks / 24 versioned assignments / 4 proven cross-state versions / 0 effect writes / 0 skipped assignments and prunes 6 now-unused physical scalar declarations; `output/24.beta.lua` contains zero `r1`..`r6` identifiers and the VM declaration is only `local ReturnVal`. A focused synthetic split-loop regression proves this without sample-specific state/register IDs. The beta file is still not directly executable because cross-state epoch locals are declared inside one dispatcher branch and referenced from sibling states (`r_v1_2` is nil/global at runtime); that lexical-hoisting issue is separate from physical-register versioning. Per user request, beta-CF has not been run on sample 24. Scheduler checkpoint `ecc6075` additionally moves the two direct `state = 2` jumps in states 1 and 3 to the physical end of their leaves; the split numeric-for branch in state 2 is intentionally unchanged. Scheduler regression, beta register/upvalue regressions, `git diff --check`, and sample-24 normal LuaJIT runtime all pass after this change.
