...
Latest beta behavior checkpoint is `0dc783f Handle beta shared continuation branches` on `main`, pushed to `origin/main`. Beta CF now structures closed, single-entry, acyclic multi-state CFGs including branches with a shared continuation plus early-return escape paths. It still parses only proven normalized transitions, removes direct state jumps into structured fallthrough, converts `condition and trueState or falseState` into source `if`, uses post-dominators when they prove a full join, and supplements them with acyclic reachability to find the earliest unique shared continuation when some paths return before that continuation. Region emission tracks whether an arm reaches its surrounding boundary rather than requiring every arm to do so, which prevents shared continuation blocks from being duplicated. Proven terminal `ReturnVal = { ... }; state = nil` pairs still lower to Lua `return`. Loops/backedges, multiple entries, unsupported operations, ambiguous incomparable continuations, unsafe terminal returns, and beta locals that would escape a new Lua branch scope fail closed. Beta remains analysis/presentation-only and source lexical-scope reconstruction is still NOT implemented. All tracked fixtures `sample/1` through `sample/17` remain paired.
...
- Every tracked `sample/N.txt` must have a matching tracked `sample/N.source.lua`. Current paired coverage is samples 1-17.
...
- Current paired fixture coverage is samples 1-17. New sample 5 schedules 390 blocks with 221,956 dependency-safe swaps, 217 unread sinks, and a structurally proven 23-slot overflow bank. Sample 10 remains the natural overflow-boundary fixture with 19 slots.
...
- Beta control-flow presentation lives in `passes/beta-control-flow.js` with CLI `node tools/beta-control-flow.js <output.lua> [output.beta.cf.lua]`. Single-state terminal CFGs keep the original flattening path. Multi-state support covers closed, single-entry, acyclic CFGs: direct jumps are treated as logical terminators and removed into fallthrough even when their textual assignment appears before later block work; two-way normalized transitions require a precomputed beta-register condition; post-dominators identify full branch joins; and acyclic reachability now detects the earliest unique partial/shared continuation when one nested arm returns before the continuation. Region emission records whether each arm reaches the region boundary, so `if/elseif`-style shared continuations are emitted once instead of duplicated. One-empty-arm joins become `if`; two non-empty arms become `if/else`; terminating arms become guard returns; every terminal block must lower to a proven compiler return. The solver rejects loops/backedges, multiple entries, missing targets, unsupported operations, ambiguous incomparable continuations, unsafe terminal returns, and branch-local beta declarations that would become out of scope. Headers remain a structural `_env` provider when required plus `local args = { ... }`.
...
- Focused regression `tools/test-semantic-names.js` covers `InitialArgs`, fixed closure `argN`, `gcProxy`, `closure`, `captureIndex`, `upvalueId`, `proxy`, `proxyMetatable`, and `RegisterOverflow`. Current paired fixtures 1-17 remain parse/runtime-valid where executable.
...
- Latest pushed beta behavior checkpoint at this snapshot: `0dc783f Handle beta shared continuation branches`; beta CF now structures closed single-entry acyclic graphs with post-dominator joins, partial/shared continuations across early-return paths, and guard-return branches while loops/backedges still fail closed.
...
- Latest fixture checkpoint: `18925b0 Add anonymous closure sample 17`.
...
- Sample 17: paired anonymous-function / closure-return fixture from `print("1"); local a = function(c,d) print("A"); return c+d end; print(2); print(a(1,2))`. It follows readable source -> local WeAreDevs Medium -> formatter. Main resolves exactly 2/2 states across two VM functions: root `createClosure` state 1 and fixed `createClosure4` state 2 for the two-source-argument anonymous function (the helper wrapper itself has four fixed VM-call parameters). Beta register versioning only was run, not beta CF: 2 state blocks, 25 versioned assignments, 4 preserved final state/return writes, 1 terminal-return placement move, 0 cross-state versions, 0 skips, 10 register epochs, and 1/1 cleanup. `output/17.beta.lua` remains dispatcher-form beta output and executes successfully. Readable source, formatted obfuscation, normal deobfuscation, and beta-only output all print `1`, `2`, `A`, `3` in that order.
