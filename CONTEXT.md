...
- closure creation inside proven `while` bodies is now integrated with closure-entry recovery. A real 5-state Medium fixture that recreates a captured closure each iteration and forwards `closure(i)` through the compiler `{ call(...) }` + final `unpack(pack)` transport passes exact runtime parity and is tracked in `tools/test-beta-control-flow-while.js`; the test also asserts no closure declaration leaks before the reconstructed `while`. A deep Roblox-shaped fixture using `game:GetService("Players").LocalPlayer.Character`, `character.Deep.A.B.C`, `character.Stats.*`, closures returning closures, nested `if/elseif/else`, break/continue/return, and closure creation inside the loop recovers across 47-state root, 48-state local-function wrapper, and 49-state local mocked-global variants. Randomized Medium certification passed 100/100 layouts total: 25/25 exact runtime parity on the 5-state minimal plus 25/25 structural recovery for each 47/48/49-state deep variant. A 10-run timing sample on the 47-state deep root measured deobf+CF median 123.7 ms (118.2 ms min, 166.1 ms max). Numeric/generic `for` remain fail-closed. The 12 established Fresh-CF/if/register/binding/upvalue/version/graph/semantic suites plus the focused while suite, text-edit suite, and batch-parameter-rename suite all pass. The old `tools/test-beta-control-flow.js:31` legacy API assertion remains the same pre-existing failure.
...
- some complex no-`continue` while bodies can still fail at an existing N-way conditional convergence shape where several continuing `elseif` arms rejoin after sibling return/break arms; this is a conditional-join limitation, not a closure/capture limitation, and remains fail-closed
- `break` / `continue` outside a proven compiler while region or with ambiguous loop-control edges
