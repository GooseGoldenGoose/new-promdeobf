...
Rust dead-call statement separator fix: when an unused local preserves a call whose emitted text begins with `(` (for example `(game:GetService(...)):Set3dRenderingEnabled(false)`), `DeadCallResult` now prefixes `;` so the preserved side effect remains a valid standalone Luau statement. Exact `output/spacial3.beta.cf.lua` now optimizes successfully in 33 ms / 14 rounds with 127 decoded strings; pass 2 is 0 edits and byte-identical (SHA-256 `3F073D3DDC3FD6097DA5CD9D0A830292D389EA5228A5B592B5F504BA678908F`); Rust suite 141/141 PASS.
Rust namecall argument-snapshot fusion: during proven Prometheus namecall recovery, a contiguous ordered gap of sole-use Index snapshot locals feeding consecutive call arguments is folded into the recovered namecall in the same transform. This preserves method-lookup -> argument-effect order because the original method capture precedes the gap; a non-direct/leading argument use is accepted only for the final moved producer. Effectful/non-local gaps and early partial uses fail closed, and generic already-recovered namecalls (including the JumpToIndex snapshot case) remain conservative. Exact TeleportToPlaceInstance split with compiler base alias now emits `r_v4_874:TeleportToPlaceInstance(game.PlaceId, game.JobId, game.Players.LocalPlayer)`; pass 2 is 0 edits; Rust suite 145/145 PASS; no big-file run.
