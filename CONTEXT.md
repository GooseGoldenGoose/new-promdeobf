...
- Tracked `sample/7.source.lua` / `sample/7.txt` is the dedicated lexical-shadowing regression. Source semantics: outer `local b = 3`; `local a = 1`; an inner `do ... end` shadows with a distinct `local b = 2`; after the block `print(b)` must resolve back to outer `b = 3`; nested function `w()` reads captured `a` and returns `1`. Readable source, Prometheus Medium fixture, and current deobfuscated output all print `1`, `2`, `3`, `1` and exit 0.
- Sample 7 still resolves 2/2 VM states: root state `1` and `w` as `createClosure5` state `2`. In the current normalized fixture, root-state data flow is explicit: `state = 3; a = state` is the outer source `b = 3`; `J = allocUpvalue(); state = 1; upvalueValues[J] = state` is captured source `a = 1`; `T = upvalueValues[J]` feeds `print(a)`; `state = 2; T = state` is the inner shadowed `b = 2`; and the later `print(b)` reads the preserved outer value from VM register `a`, not `T`. These register names are fixture evidence only and must never become implementation assumptions.
- Sample-7 closure lifetime proof: `createClosure5(2, {J})` calls `createUpvalueProxy`, which increments the captured cell reference count; the root then `releaseUpvalue(J)`s its own reference, leaving the closure-owned reference alive. State 2 reads the same cell through `upvalueValues[upvalues[1]]` and returns source `1` via the VM return table. Thus source `w` captures exactly the `a` cell, while both `b` bindings remain ordinary non-captured values.
...
- Shared-cell identity is now explicit: sample 3's two closures resolve to the same local cell and all 5 cell accesses resolve to it; sample 4's three closures resolve to the same `total` cell and all 17 accesses resolve; sample 6 resolves the root->outer and outer->inner captures to two different cells with all 14 accesses resolved; sample 7 resolves one captured cell, all 22 tracked VM-register uses uniquely reach a definition, and all 3 cell accesses resolve to that same cell.
...
- Definition-site liveness is now computed backward over each recovered function CFG using the reaching-definition identities. Each definition records use counts, live-in/live-out blocks, cross-block lifetime, and loop-carried status. Alternative definitions are placed in a conservative join component only when they co-reach the same identifier use; sequential register reuse is not merged. Sample 4 has one such join component and one loop-carried lifetime; sample 6 has cross-block lifetimes; sample 7 has no ambiguous uses or forced join components; sample 5 converges with 177 join components, 695 cross-block lifetimes, and 61 loop-carried lifetimes.
- Captured source-binding candidates now exist as a separate proof class keyed by static `allocUpvalue()` definition identity. Every candidate summarizes its owning function, resolved `upvalueValues` reads/writes, capture consumers, and transitive relays. Capture edges themselves now carry final `resolvedCellId` when provable. Samples 3/4/6/7 have complete cell graphs and therefore complete captured-cell candidates; sample 5 remains fail-closed with 92 unresolved capture edges plus 182 unresolved cell accesses, so its 112 candidates are marked provenance-incomplete and are not eligible for source rewriting. Transitive relay resolution increases the large-sample shared-cell count because grandchildren consuming the same cell are now recognized structurally.
- `main.js` exposes compact reaching-definition, lifetime, cell-access, and captured-binding-candidate counts. Final source output remains exactly the Step 3 normalized source for now. Reparse still occurs after the final emitted source, and LuaJIT runtime parity was reconfirmed for samples 3, 4, 6, and 7 after all Step-4 extensions.
...
3. For ordinary locals, build conservative register-value epochs from definition/use/liveness connectivity without merging sequential unrelated values. Sample 7 remains the primary shadowing regression; only promote a source local when function ownership, provenance, and lifetime proof are unique.
