...
- Local-nil initialization proof from the local WeAreDevs compiler: `local x` and `local x = nil` both compile a `NilExpression` into a temporary register, then `getVarRegister(scope,id,...,potentialId)` promotes that same temporary register into the source local's reserved `VAR_REGISTER`. `copyRegisters()` emits nothing when source and destination are the same register, so the VM can begin an ordinary local with a single direct `R = nil`. By contrast, a later source assignment `x = nil` usually compiles through a fresh temporary (`tmp = nil; xReg = tmp`) because `xReg` is already reserved. Therefore direct `R = nil` is not inherently a scope-end marker: it can also be the initialization of a new local. Existing binding-end analysis remains conservative because it requires a prior reaching definition with uses before treating a dead direct-nil write as an end candidate.
