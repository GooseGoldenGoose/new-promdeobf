...
Dead state/POS initializer cleanup runs after dead snapshot cleanup. For a surviving `local x = state` whose beta binding is reused later, it propagates that specific initial definition through the state CFG and removes only the initializer when every path reaches an overwrite/end before a semantic read. Dead copy chains are ignored/removed only when they are actually reached by that old definition before its kill. The declaration is retained as `local x` so later writes keep the same lexical binding. Reads before overwrite, alternate-path reads, missing/ambiguous successors, and live copy chains fail closed.
...
Dead state/POS initializer verification: after the first snapshot pass leaves 8 `spacial6` initialized state snapshots, the new definition-level pass converts 6 dead initial values to bare locals and leaves 2 initialized snapshots because their saved state value still flows into live beta values. Dedicated dead-initializer/dead-copy and live-read regressions pass. Full project suites remain 14/14, canonical numeric CF remains 63/63, and `spacial6` remains 3799 states / 554 closures.
