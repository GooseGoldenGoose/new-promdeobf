...
- The scheduler compacts both producer -> consumer and write -> next-touch gaps. For a pure one-register assignment it finds the next read OR next write of that same register and may sink the assignment until immediately before that next touch, but only across statements that are individually RAW/WAR/WAW-safe. This keeps a producer close to its use and also groups overwrite chains such as `A = 1; ...; A = 2` into `...; A = 1; A = 2` without deleting either write. Consumer-gap compaction still pushes unrelated pure fillers out of a direct producer/use chain. Each original statement is scheduled at most once to prevent oscillation.
...
- Sample 8 exposes the current behavior clearly: `D = "keep1"` is next to `i = l(D, e)`; `B = "print"` now sinks directly above `G = _env[B]`; `F = "inner"` stays next to `B = G(F, D)`; and the ownership-handoff pair is preserved as adjacent `D = nil; D = 30`. The scheduler intentionally does not remove overwritten pure stores; it only compacts them.
- Current scheduling counts after next-touch compaction: sample 1 -> 1 block / 5 swaps; sample 2 -> 1 block / 4 swaps; sample 3 -> 1 block / 62 swaps; sample 4 -> 6 blocks / 36 swaps; sample 6 -> 6 blocks / 47 swaps; sample 7 -> 3 blocks / 25 swaps; sample 8 -> 1 block / 133 swaps. All outputs reparse. LuaJIT runtime parity matches for samples 1/2/3/4/6/8 and both deterministic sample-7 branches.
