...
- Current tracked `sample/7.source.lua` was edited to put `return` inside the inner `do ... end` immediately after `print(b)`. Therefore the later outer `print(b)`, nested `w()` declaration/use, and its runtime capture are unreachable. Source, formatted Prometheus Medium fixture, and deobfuscated output all print only `1`, `2` and exit 0.
