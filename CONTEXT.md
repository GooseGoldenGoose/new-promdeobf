...
Latest beta behavior checkpoint is `d2ad98b Lower beta terminal return payloads` on `main`, pushed to `origin/main`. Beta keeps the sparse data-flow register-epoch model and the one-state control-flow presentation stage. A structurally proven terminal compiler return payload is now lowered from `ReturnVal = { ... }; state = nil` into a real Lua `return ...`; empty payloads become bare `return`, and a final `unpack(register)` entry remains `unpack(register)` so multi-return behavior is preserved. Non-empty return payloads are never sunk across effectful/unsafe operations or writes to returned versions. Multi-state CFG structuring is still not implemented. Beta remains analysis/presentation-only. All tracked fixtures `sample/1` through `sample/13` have matching `.source.lua` + formatter-normalized `.txt` pairs. Source lexical-scope reconstruction has NOT been implemented yet.
