...
Latest implementation checkpoint is `a265716 Recover while loops with guard breaks` on `main`, pushed to `origin/main`; latest fixture checkpoint is `4d100bd Add while-loop sample 27`. Beta-CF supports compiler-proven numeric `for` loops plus pre-test `while` natural loops emitted in guard-break form. Sample 27 now structures successfully as one while loop with exact runtime parity. All tracked fixtures `sample/1` through `sample/27` remain paired.
...
- Latest pushed implementation checkpoint at this snapshot: `a265716 Recover while loops with guard breaks`; latest normal-pipeline optimization is `11a5629 Optimize deobfuscation pipeline passes`; latest fixture checkpoint is `4d100bd Add while-loop sample 27`. Beta-CF supports compiler-proven numeric `for` loops and pre-test `while` natural loops. `while` conditions remain as beta operations inside `while true`, followed by `if not (<condition>) then break end`, preserving per-iteration evaluation. Unmatched cycles/repeat loops still fail closed. Terminal compiler return payloads, nested-child return safety, captured-cell lexical ordering, ambiguous assignments, unresolved captures/cells, and multi-state cell-owner safeguards remain enforced.
