...
Latest beta behavior checkpoint is `e2cace8 Inline empty-capture beta closure regions` on `main`, pushed to `origin/main`. Beta CF still structures closed acyclic state graphs with full joins, shared continuations, and early-return escapes, and it now also partitions multiple VM entries into structurally owned closure regions. A unique root entry is the entry not created by any in-state `createClosure*` call; each child entry must have one unique parent factory site. Child regions are solved bottom-up with the same single-entry solver used for the root, then an exact empty-capture `createClosure(entry, {})` / `createClosureN(entry, {})` assignment is replaced by a nested `function(...)` containing `local args = { ... }` and the solved child body. Proven child `ReturnVal = { ... }; state = nil` tails become ordinary Lua `return`. Non-empty capture lists, cross-entry CFG edges, overlapping entry ownership, multiple structural parents, loops/backedges, unsupported operations, ambiguous continuations, and unsafe scope movement still fail closed. Beta remains analysis/presentation-only and source lexical-scope reconstruction is still NOT implemented. All tracked fixtures `sample/1` through `sample/18` remain paired.
...
- Every tracked `sample/N.txt` must have a matching tracked `sample/N.source.lua`. Current paired coverage is samples 1-18.
...
- Current paired fixture coverage is samples 1-18. New sample 5 schedules 390 blocks with 221,956 dependency-safe swaps, 217 unread sinks, and a structurally proven 23-slot overflow bank. Sample 10 remains the natural overflow-boundary fixture with 19 slots.
...
- Beta control-flow presentation lives in `passes/beta-control-flow.js` with CLI `node tools/beta-control-flow.js <output.lua> [output.beta.cf.lua]`. Single-entry regions still use the existing single-state or acyclic structurer: direct jumps become fallthrough, normalized two-way transitions become `if`, post-dominators find full joins, acyclic reachability finds unique shared continuations across early-return paths, and proven terminal compiler returns become Lua `return`. Multiple VM entries are now supported when they can be partitioned into disjoint closure-owned regions. In-state exact `createClosure` / `createClosureN` calls identify parent-to-child entry edges; the one entry not referenced by such a site is the root. Child regions are solved bottom-up by the same single-entry solver and empty-capture factory assignments are replaced with nested `function(...)` values whose local `args = { ... }` feeds the child VM-argument reads. Each child entry must have exactly one structural parent; region overlap or cross-entry CFG transitions reject the transform. Capture reconstruction is deliberately not implemented yet, so any non-empty closure capture table fails closed rather than emitting a false lexical closure. CLI metadata reports closure-region and inlined-factory counts when this mode applies.
...
- Focused regression `tools/test-semantic-names.js` covers `InitialArgs`, fixed closure `argN`, `gcProxy`, `closure`, `captureIndex`, `upvalueId`, `proxy`, `proxyMetatable`, and `RegisterOverflow`. Current paired fixtures 1-18 remain parse/runtime-valid where executable.
...
- Latest pushed beta behavior checkpoint at this snapshot: `e2cace8 Inline empty-capture beta closure regions`; beta CF now handles both single-entry acyclic state graphs and multiple disjoint closure-entry regions when child factory sites are structurally proven and capture lists are empty. Captured closures and loops/backedges still fail closed.
...
- Latest fixture checkpoint: `1960b1d Add captured upvalue sample 18`.
