...
Latest implementation checkpoint is `b3d7aa3 Recover repeat-until control flow` on `main`, pushed to `origin/main`; latest fixture checkpoint is `e8b949c Add loop edge-case samples 31-38`. Beta-CF supports compiler-proven numeric `for`, pre-test `while` in guard-break form, and post-test `repeat ... until`. Repeat recovery removes the local compiler's discarded first condition evaluation to restore readable-source semantics. Sample 30 proves mixed nesting across repeat/for/while, branches, do blocks, and nested functions. All tracked fixtures `sample/1` through `sample/38` remain paired.
...
- Current paired fixture coverage is samples 1-38. New sample 5 schedules 390 blocks with 221,956 dependency-safe swaps, 217 unread sinks, and a structurally proven 23-slot overflow bank. Sample 10 remains the natural overflow-boundary fixture with 19 slots.
...
- Numeric `for` recovery: `passes/beta-control-flow.js` detects the exact local WeAreDevs `ForStatement` topology/data flow rather than arbitrary cycles. It requires one preheader -> check edge, the compiler sign test (`step < 0`), current initialization (`start - step`), current increment, paired `<=`/`>=` limit tests, split POS targets, the loop-variable copy, and one matching cleanup on a proven body latch. The body may now contain an acyclic internal CFG region with `if/else` branches and joins; all body-region entries must be owned by the loop, every body path must return to the check, and no body edge may escape to the loop exit or another cycle. On proof, the whole body region is structured recursively inside one numeric-for node. Multiple sequential numeric-for regions are collapsed iteratively. Any unmatched check/body operation, external entry, ambiguous join, loop-variable rewrite, or unrelated cycle remains fail-closed.
...
- Captured-cell lexical declaration order is now preserved when the local compiler creates a child closure before emitting the captured cell's first value initialization (captured parameters are a real case). If the first factory capture precedes the proven initialization in a single-state owner region, beta upvalue recovery emits only `local <binding>` at the original `allocUpvalue()` position, rewrites the later cell initialization to `<binding> = <value>` at its original position, and empties the closure capture table as usual. This guarantees the already-created nested Lua function lexically captures the recovered local instead of accidentally referring to a global/nil binding. Other cell recovery modes remain unchanged and ambiguous/multi-state ownership still fails closed.
...
5. Extend beta CFG beyond the proven numeric-`for`, pre-test `while`, and post-test `repeat` handlers using focused samples 31-38: explicit `break` exits (31), numeric-for `continue` plus nested loop-control exits (32-33), early `return` from loop bodies (34), multi-state captured-cell dominance inside loops (35), multi-state short-circuit loop conditions (36), and source mutation of the numeric-for loop variable (37). Sample 38 is the positive deeply-branched-body regression. Keep all unsupported shapes fail-closed.
...
- Latest pushed implementation checkpoint at this snapshot: `b3d7aa3 Recover repeat-until control flow`; latest normal-pipeline optimization is `11a5629 Optimize deobfuscation pipeline passes`; latest fixture checkpoint is `e8b949c Add loop edge-case samples 31-38`. Beta-CF supports compiler-proven numeric `for`, pre-test `while` natural loops, and post-test `repeat ... until`. `while` conditions remain inside `while true` with a guard break. `repeat` emits a real post-test loop and structurally removes the local compiler's duplicated discarded preheader condition evaluation to restore source semantics. Other unmatched/general cycles still fail closed. Terminal compiler return payloads, nested-child return safety, captured-cell lexical ordering, ambiguous assignments, unresolved captures/cells, and multi-state cell-owner safeguards remain enforced.
- Indexed/member effect-write support from `f16b244` remains active and is consumed normally by beta upvalue recovery/CF.
- Latest fixture checkpoint is `e8b949c Add loop edge-case samples 31-38`; latest numeric-for CF checkpoint is `5f7c7f4 Recover numeric for loops with branched bodies`; tracked samples 1-38 remain paired.
...
- Samples 31-38: focused post-loop-structuring edge-case matrix (generic iterator `for ... in` intentionally excluded by user request). All eight sources compile through local WeAreDevs Medium + formatter, normal deobfuscation succeeds, beta register versioning succeeds with 0 skipped assignments, and raw/formatted/normal runtime parity holds for every fixture. `continue` sources (32-33) parse with the project Luau parser and compile through Prometheus; local LuaJIT cannot execute readable Luau `continue` syntax directly, so runtime parity there is raw/formatted/normal.
  - Sample 31 (`break` in while/numeric-for/repeat): 16/16 states, 83 reaching definitions, 3 loop-carried lifetimes. Source/raw/formatted/normal stdout are identical (131 bytes). Beta-CF fails closed; partial structuring collapses 0 loops. Current loop-body collectors reject the deliberate body -> loop-exit edge, so explicit `break` is not yet supported for any of the three loop kinds.
  - Sample 32 (`continue` in numeric-for/while/repeat): 16/16 states, 90 defs, 4 loop-carried lifetimes. Raw/formatted/normal stdout are identical (219 bytes). Partial beta-CF collapse proves the existing while and repeat handlers can consume their continue-shaped early latch paths (1 while + 1 repeat collapse), but numeric-for with continue remains cyclic/unmatched and prevents complete CF output. Treat continue as not fully supported until numeric-for and mixed full-output cases pass.
  - Sample 33 (nested for/while/repeat with both break and continue): 17/18 reachable states (1 dead), 78 defs, 2 loop-carried lifetimes. Raw/formatted/normal stdout are identical (216 bytes). No loop collapses; beta-CF fails closed. This is the loop-target ownership regression for future nested break/continue recovery.
  - Sample 34 (early `return` from while/numeric-for/repeat, each in a separate function): 19/19 states across 4 VM functions, 99 defs, 3 loop-carried lifetimes. Source/raw/formatted/normal stdout are identical (26 bytes). No loop collapses; beta-CF fails closed because current loop-body proof requires normal paths to stay in the loop until latch/check and does not yet model a body edge to a terminal return region.
  - Sample 35 (closures capturing loop/live locals inside numeric-for/while/repeat): 13/13 states across 4 VM functions, 4 capture slots, 4 local cells, 16/16 upvalue cell accesses resolved, and 4 captured-binding candidates. Source/raw/formatted/normal stdout are identical (179 bytes). Beta-CF fails before final structuring with the existing `Captured cell ... is allocated in a multi-state owner region; lexical dominance proof is not implemented yet` safeguard. Partial loop collapse can reduce the while and repeat shapes, but complete captured-loop closure recovery remains unsupported.
  - Sample 36 (side-effectful short-circuit conditions in while and repeat): 25/25 states across 7 VM functions, 222 defs, 5 joins. Neither loop collapses because condition evaluation spans multiple CFG states; current loop matchers require a single branch header/check state. Source output is 219 bytes while raw Prometheus/formatted/normal are 198 bytes because the already-known Prometheus RepeatStatement bug pre-evaluates/discards the repeat condition once; raw/formatted/normal remain exact to each other. Future short-circuit loop recovery must structure the condition CFG itself and, for repeat, still remove the compiler-added first condition evaluation to restore source semantics.
  - Sample 37 (assignment to numeric-for source loop variable): 4/4 states, 45 defs, 1 loop-carried lifetime. Source/raw/formatted/normal stdout are identical (136 bytes). Numeric-for collapse intentionally rejects the additional loop-variable write, so beta-CF fails closed. This remains the focused regression before supporting mutable source loop variables without confusing them with compiler current/loop-var machinery.
  - Sample 38 (deep nested if/else/elseif-like branches and tails inside while/numeric-for/repeat): 33/33 states, 155 defs, 3 loop-carried lifetimes. Beta-CF succeeds with 1 numeric-for + 1 while + 1 repeat, 8 structured branches, and 1 terminal return. Source/raw/formatted/normal/beta-CF stdout are byte-identical (303 bytes). This proves complex acyclic branch/join bodies are already supported and are not a remaining loop blocker.
