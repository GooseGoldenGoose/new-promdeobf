...
Verified on the user opcode/closure/TCO fixture: proven root `upvalueValues[cell]` reads and bare `cell = allocUpvalue()` allocations can both cross pending compiler multi-return pack bookkeeping without forcing an early flush. `allocUpvalue()` is accepted only for a fresh ordinary destination, zero arguments, and never `state`/`ReturnVal`; arbitrary calls remain barriers. The previous `r53 = allocUpvalue()` blocker is fixed. The fixture now reaches source emission successfully. An earlier shell display made `local v45 = (-v20)` and `local v49 = ...` appear concatenated, but the saved `.beta.cf.lua` contains proper newlines and parses successfully; there was no solver newline bug.
