...
Repeat recovery supports loop-carried locals, ordinary shared block semantics, nested `if/elseif/else`, `break`, Luau `continue`, early `return`, nested repeat, repeat-inside-while, while-inside-repeat, and closure-contained repeat. `continue` targets the repeat condition-region entry so the `until` condition is still evaluated. `break` is accepted only through a closed compiler cleanup/jump region to the same repeat final; return exits must be closed compiler terminal-return regions. Ambiguous side exits fail closed. `passes/beta-cf/control/loops.js` discovers natural loops, classifies the smallest loop as numeric `for`, `repeat`, or `while`, collapses it, recomputes the graph, and repeats inner-first so nested/mixed loops share one structured solver. `passes/beta-cf/control/loop-storage.js` owns shared loop-carried source-storage proof and remaps start evidence by AST statement identity after CFG rewrites, avoiding stale preheader indexes. Numeric-for has its own compiler signature proof; generic-for remains unsupported and must not be misclassified as while/repeat/numeric-for.
...
Fresh CF now recovers proven Prometheus numeric `for` loops in `passes/beta-cf/control/numeric-for.js`. Before ordinary `StateGraph` construction, the classifier canonicalizes only the exact compiler split transition `state = cond and body; state = state or final` into one synthetic branch, then proves the preheader final/step/`step < 0`/zero/current setup, the sign-selected `current <= final` or `current >= final` check, and exactly one compiler current-to-source-loop-variable handoff in the body. The compiler bookkeeping is removed while the source loop variable is reintroduced as a path-local binding at body entry. Numeric-for bodies reuse the same structured statement/lifetime/branch/upvalue/closure engine, including nested `if/elseif/else`, loop-carried locals, Luau `continue`, proven cleanup `break`, closed early `return` regions, nested numeric loops, and closure-contained numeric loops. Bounds and step are recovered dynamically, including negative, non-unit, and variable expressions. Prometheus emits the same VM pattern for omitted step and explicit step `1`, so Fresh CF cannot prove which surface form was authored; proven step `1` canonicalizes to `for v = initial, final do`, while other steps remain explicit. Ambiguous setup, extra check statements, unproven side exits, missing/ambiguous loop-variable handoff, or non-matching sign logic fail closed.
...
- `passes/beta-cf/control/loops.js`: shared natural-loop orchestrator. It repeatedly discovers reachable natural loops, classifies/collapses the smallest proven numeric-for/repeat/while loop inner-first, combines construct metadata, and invokes the structured solver once for pure or mixed nesting.
- `passes/beta-cf/control/numeric-for.js`: strict Prometheus numeric-for split-check canonicalization, sign-aware preheader/check/body-handoff proof, numeric-loop CFG rewriting, and reuse of proven abrupt-region classification.
- `passes/beta-cf/control/while.js`: compiler pre-test `while` signature proof, while break/continue/return classification, CFG rewriting helpers, natural-loop/dominator utilities, and shared proven abrupt-region helpers; while itself still anti-matches numeric/generic-for shapes.
...
- `passes/beta-cf/control/loop-storage.js`: shared loop-carried source-storage evidence and post-rewrite statement-identity remapping.
...
- proven Prometheus numeric-for/while/repeat, nested or mixed loops, and loop break+continue -> `passes/beta-cf/control/loops.js` orchestrating `control/numeric-for.js` / `control/while.js` / `control/repeat.js` plus shared structured lifetime/branch rendering
- focused behavior regressions -> `tools/test-beta-control-flow-fresh.js`, `tools/test-beta-control-flow-if.js`, `tools/test-beta-control-flow-while.js`, `tools/test-beta-control-flow-repeat.js`, and `tools/test-beta-control-flow-numeric-for.js`; register, binding, upvalue, version, graph, and semantic-name regressions remain in their matching `tools/test-*.js` suites
...
Numeric-for support validated on 2026-09-05: `control/numeric-for.js` proves the real Prometheus split check plus preheader sign/current setup and source loop-variable handoff, then reuses the unified structured solver for the body. Permanent focused coverage includes canonical step `1`, descending `-1`, non-unit step `2`, Luau `continue`, and proven `break`. Real Medium fixtures additionally certify variable bounds/step, nested `if/else` with loop-carried storage, early return, nested numeric loops, and closure-contained numeric loops. Explicit `, 1` and omitted step compile identically and therefore canonicalize to the no-step source form. The expanded regression gate passes 17/17. Randomized Medium stress passed 25 fresh layouts across each of eight exact-runtime fixture families = 200/200 source/obfuscated/recovered runtime parity, with median deobf+CF 52.8 ms. A real generic-for probe remains fail-closed and is not misclassified as numeric-for.

Loop recovery is modular and unified at orchestration level. `control/loops.js` owns natural-loop classification/order and composes metadata from `control/numeric-for.js`, `control/while.js`, and `control/repeat.js`; all supported constructs reuse the same structured statement/lifetime/branch engine. Generic-for should follow the same dedicated-classifier pattern rather than duplicating statement semantics or private `elseif`, pack, upvalue, or closure logic. Unsupported/ambiguous loop shapes continue to fail closed.

Before adding any new recovery feature, run syntax checks for every changed/new JavaScript file, the established 17-suite Fresh-CF/if/while/repeat/numeric-for/register/binding/upvalue/version/graph/semantic gate, relevant real Medium runtime-parity fixtures, randomized layout stress for the touched shape, and `git diff --check`.
...
7. try closure-entry program recovery; root and child closure entries delegate proven numeric-for/while/repeat loops through the same unified loop orchestrator with root-only reachability and the existing transactional child-closure renderer
8. try unified structured loop recovery (numeric `for`, `while`, `repeat`, nested/mixed loops) before generic logical/conditional recovery
...
- arbitrary/unproven `repeat` CFGs outside the compiler post-test/duplicate-condition proof
- arbitrary/unproven numeric `for` CFGs outside the strict Prometheus numeric-for signature proof
- generic `for`
...
- arbitrary/unproven numeric `for` CFGs outside the strict Prometheus numeric-for signature proof
- generic `for`
- arbitrary nested CFG structuring outside the proven conditional/numeric-for/while/repeat composition
...
- `break` / `continue` outside a proven compiler loop region or with ambiguous loop-control edges
...
StateGraph
  ...
  -> classify as numeric-for, repeat, or while
  -> collapse only that loop / record loop metadata
  -> recompute graph
  -> repeat inner-first
  -> run one shared structured solver
```
Files: `control/loops.js` = orchestrator; `control/numeric-for.js` = numeric-for split-check/sign/preheader/body-handoff proof; `control/while.js` = while compiler signature/proofs plus shared abrupt-region helpers; `control/repeat.js` = repeat signature + duplicated-condition proof/removal; `control/loop-storage.js` = shared loop-carried-storage evidence and statement-identity remapping after CFG rewrites. Closure child-loop recovery delegates through this same orchestrator.

Current supported core includes straight line, if/else/elseif, nested conditionals, numeric `for`, while, repeat, nested/mixed proven loops, break, continue, early return, globals, stable table reads/writes, calls/namecalls, closures, upvalues, multi-return, varargs, and logical and/or, subject to existing fail-closed proof requirements.

Still explicitly unsupported/fail-closed: general generic `for`, arbitrary/unproven numeric-for variants outside the certified Prometheus template, and arbitrary/ambiguous CFG or loop shapes that do not match proven Prometheus compiler structure. Do not weaken safety or let one loop classifier consume another construct by guesswork.
