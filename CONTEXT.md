...
- `main.js` exposes the binding-analysis result and compact cell-access resolution counts. Final source output remains exactly the Step 3 normalized source for now. Reparse still occurs after the final emitted source, and LuaJIT runtime parity was reconfirmed for samples 3, 4, 6, and 7 after the access-provenance change.
...
2. Add liveness/lifetime grouping over definition identities so ordinary locals and shadowed bindings can be separated without inventing locals. Use sample 7 as the primary regression and require unique def/use proof before source-like local recovery.
3. Relate register-definition lifetimes to resolved cell accesses and closure captures so a future binding candidate can distinguish an ordinary value lifetime from a mutable captured source binding.
