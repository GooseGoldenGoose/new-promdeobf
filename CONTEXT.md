...
Latest beta/CF behavior checkpoint is `5f7c7f4 Recover numeric for loops with branched bodies` on `main`, pushed to `origin/main`; fixture checkpoint `48daa23 Add mixed numeric for sample 25` is also pushed. Beta-CF structurally recognizes the local WeAreDevs compiler numeric `ForStatement` lowering and now allows the loop body to be an acyclic internal CFG region with branches/joins before one or more proven backedge latches. The matcher still requires the compiler preheader/sign/check/current/loop-variable/cleanup topology and fails closed on unrelated cycles or escaping body edges. Terminal-return and captured-binding protections remain unchanged. All tracked fixtures `sample/1` through `sample/25` remain paired.
...
- Latest pushed behavior checkpoint at this snapshot: `5f7c7f4 Recover numeric for loops with branched bodies`; latest fixture checkpoint is `48daa23 Add mixed numeric for sample 25`; earlier numeric-for checkpoint is `08b53ca Recover Prometheus numeric for loops`. Beta-CF supports compiler-proven numeric `for` loops while unrelated/general cycles still fail closed. Terminal compiler return payloads, nested-child return safety, captured-cell lexical ordering, ambiguous assignments, unresolved captures/cells, and multi-state cell-owner safeguards remain enforced.
- Indexed/member effect-write support from `f16b244` remains active and is consumed normally by beta upvalue recovery/CF.
- Latest fixture checkpoint is `48daa23 Add mixed numeric for sample 25`; latest numeric-for CF checkpoint is `5f7c7f4 Recover numeric for loops with branched bodies`; tracked samples 1-25 remain paired.
