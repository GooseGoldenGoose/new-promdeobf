...
Latest implementation checkpoint is `b3d7aa3 Recover repeat-until control flow` on `main`, pushed to `origin/main`; latest fixture checkpoint is `b3efcbe Add mixed repeat sample 30`. Beta-CF supports compiler-proven numeric `for`, pre-test `while` in guard-break form, and post-test `repeat ... until`. Repeat recovery removes the local compiler's discarded first condition evaluation to restore readable-source semantics. Sample 30 proves mixed nesting across repeat/for/while, branches, do blocks, and nested functions. All tracked fixtures `sample/1` through `sample/30` remain paired.
...
- Every tracked `sample/N.txt` must have a matching tracked `sample/N.source.lua`. Current paired coverage is samples 1-30.
...
- Current paired fixture coverage is samples 1-30. New sample 5 schedules 390 blocks with 221,956 dependency-safe swaps, 217 unread sinks, and a structurally proven 23-slot overflow bank. Sample 10 remains the natural overflow-boundary fixture with 19 slots.
...
- Focused regression `tools/test-semantic-names.js` covers `InitialArgs`, fixed closure `argN`, `gcProxy`, `closure`, `captureIndex`, `upvalueId`, `proxy`, `proxyMetatable`, and `RegisterOverflow`. Current paired fixtures 1-30 remain parse/runtime-valid where executable.
...
5. Extend beta CFG beyond the proven numeric-`for`, pre-test `while`, and post-test `repeat` handlers to explicit `break`/`continue` exits and other/general backedge shapes using natural-loop proof; sample 4 remains the focused mixed-loop regression.
