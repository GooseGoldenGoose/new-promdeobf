...
Step 31 PLANNED — closure positional-parameter / vararg signature recovery (next implementation step): current `recoverNestedFunctionSignature` only recovers fixed parameters when the solved nested body still contains explicit compiler loads such as `local r = args[1]`. Post-CF cleanup can legally inline those loads, leaving direct uses such as `return args[1]`; signature recovery then falls back to `function(...)` + `local args = { ... }`. Step 31 will extend the existing closure-signature proof, not add a generic source optimizer.

Implementation location: `passes/beta-control-flow.js`, centered on `recoverNestedFunctionSignature` / `nestedFunctionExpression`, after a child closure region is fully solved and before its recovered function expression is embedded in the parent.

Step 31 proof/behavior plan:
- Never infer source arity from the numeric suffix of `createClosureN`; the local compiler randomizes that suffix. The factory name may classify fixed-vs-vararg compiler shape only where already structurally proven, but `N` is not source parameter count.
- Parse the solved nested body and inventory every use of the synthetic `args` storage outside nested function literals.
- Accept fixed positional reads only when every surviving `args` use is an exact static positive-integer `args[K]` read, or an already-supported compiler parameter-load statement whose RHS is exactly `args[K]`.
- Reject/fallback on any dynamic index, write to `args`/`args[K]`, bare `args` escape/alias, table identity observation, unsupported `unpack(args)`/`select` use, nested capture, ambiguous declaration, or parse/range failure.
- Recover positional count from the highest proven static slot actually required by body semantics. Gaps remain positional: if only `args[2]` is used, emit two parameters so slot 2 stays slot 2.
- Reuse an existing proven parameter-load target name when one exists for that slot. For a direct-only slot, generate a deterministic collision-free presentation name such as `arg1`, `arg2`, ...; never hardcode register/sample-specific names.
- Rewrite all proven direct `args[K]` reads to that slot's recovered parameter binding, then remove the synthetic `local args = { ... }` header by emitting a recovered function signature.
- Preserve the existing vararg-tail proof. A closure with real vararg semantics must still emit `...`; direct positional slots before the proven tail may be recovered, but ambiguous/mixed vararg storage continues to fall back conservatively.
- Do not cross/move calls, assignments, or closure creation. This is a post-CF presentation/data-binding recovery over one already-structured nested function body.
- Reparse the rewritten nested body/function expression before accepting it.

Required focused tests for Step 31:
- `function(a) return a end` shape reduced to direct `return args[1]` -> `function(arg1) return arg1 end`.
- two/three fixed positional slots, including reordered reads and repeated reads of the same slot.
- sparse slot use (only slot 2) preserves positional arity.
- mixed explicit load + direct `args[K]` use.
- collision with an existing local named `arg1` forces a fresh deterministic parameter name.
- nested function mentioning its own `args` does not poison the outer proof and outer `args` is never rewritten inside the nested function.
- fail closed: dynamic `args[i]`, `args` escape, `args[K] = ...`, capture/alias, unsupported unpack/select, malformed/missing ranges.
- real vararg fixture keeps `...` and existing vararg result semantics.

Real Medium verification target: reuse/regenerate the compiler static closure audit where source `function g(a) return a end`, `function t.m(a) return a end`, and `t.n = function(a) return a end` currently produce `function(...) local args={...}; return args[1] end`. After Step 31 these live closures should have one recovered positional parameter and no synthetic `args` header, while source/obfuscated/final Luau stdout and exit status remain exact. Then run the focused signature suite, main beta-CF suite, and full PRE-CF + CF combined 1-66 gate before commit/push.
