...
Rust stable-field fallback snapshot follow-up: immediate sole-use static dot-field fallback expressions composed from stable field chains, stable lexical names, literals, and `and`/`or` may now inline into the next stable direct/namecall argument; computed indexes and effectful prefix arguments remain blocked. Immediate global aliases may also inline into a dot-field assignment target only when the RHS is effect-free. Exact `output/spacial6.beta.cf.lua`: 5287 ms / 39 rounds; `o_v121_84`, `r_v55_328`, `r_v55_317`, and nested `r_v10_1043` all drop to 0 occurrences; output includes `_G.auto_executed = true` and direct `toggle_img_url` / `ui_font` fallback arguments. Pass 2 = 0 edits and byte-identical SHA-256 `0CA2EE08F10A35E71A9880D9A727F7B8B5F1AE409DF0D0241F2EF34F27D6F6AD`; Rust suite 186/186 PASS.
