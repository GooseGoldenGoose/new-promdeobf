...
Latest beta behavior checkpoint is `8b9ffdb Merge returned local beta lifetimes` on `main`, pushed to `origin/main`. Beta keeps the sparse data-flow register-epoch model and now supports two compiler-backed ownership anchors for blind same-register copy mutations: proven lexical cleanup and a terminal compiler-shaped return that directly reads a copied ordinary register. The beta control-flow stage remains limited to one complete terminal state and emits flat `--headers` / `--body` output. Multi-state CFG structuring is not implemented yet. Beta remains analysis/presentation-only. All tracked fixtures `sample/1` through `sample/13` have matching `.source.lua` + formatter-normalized `.txt` pairs. Source lexical-scope reconstruction has NOT been implemented yet.
...
- Beta control-flow presentation now lives in `passes/beta-control-flow.js` with CLI `node tools/beta-control-flow.js <output.lua> [output.beta.cf.lua]`. The first stage deliberately accepts only a closed CFG with exactly one entry, one state, and a stop terminator; it emits `--headers`, a structurally derived environment binding such as `local _env = getfenv()` when needed, always adds root VM arguments as `local args = { ... }`, and then emits the state operations as a flat `--body`. Any multi-state graph, unsupported state operation, unknown environment provider, or non-terminal entry fails closed.
...
- Latest pushed beta behavior checkpoint at this snapshot: `8b9ffdb Merge returned local beta lifetimes`; cleanup-first grouping from `cb2a1af` stays superseded, and terminal-return anchoring is now part of the sparse data-flow epoch model.
...
- Sample 13: returned-local mutation fixture: `local a = 3; print(a); a = 4; return a`. It is source-backed through local WeAreDevs Medium + formatter, resolves to one terminal state, and has exact runtime parity: prints `3` and returns `4` in source, formatted obfuscation, and normal deobfuscated output. Beta CF now emits one local lifetime for the source-like register: `local r_v2_1 = ...`, later `r_v2_1 = ...`, then terminal `ReturnVal = { r_v2_1 }`; the reassignment is no longer split into `_2` merely because function return omitted lexical cleanup.
