...
Latest beta/CF behavior checkpoint is `08b53ca Recover Prometheus numeric for loops` on `main`, pushed to `origin/main`; preceding beta/scheduler checkpoints are `b4ad0c8 Fix beta numeric loop register flow` and `ecc6075 Move numeric VM transitions to leaf tail`. Beta-CF now structurally recognizes the local WeAreDevs compiler numeric `ForStatement` lowering: preheader setup, sign test, check block, loop-variable copy/cleanup, backedge, and final continuation are consumed into a real Lua numeric `for`. The matcher is data-flow/topology based and fails closed if the compiler signature is incomplete. Terminal-return and captured-binding protections remain unchanged. All tracked fixtures `sample/1` through `sample/24` remain paired.
...
- Numeric `for` recovery: `passes/beta-control-flow.js` now detects the exact local WeAreDevs `ForStatement` topology/data flow rather than arbitrary cycles. It requires one preheader -> check edge, the body backedge to check, the compiler sign test (`step < 0`), current initialization (`start - step`), current increment, paired `<=`/`>=` limit tests, the split POS branch targets, and the body loop-variable copy plus matching lexical cleanup. On proof, it collapses check/body states into a structured numeric-for node while preserving the evaluated start/final/step beta values and body operation order. Multiple sequential numeric-for regions are collapsed iteratively. Any extra/unmatched check operation or ambiguous topology leaves the cycle untouched so the generic acyclic stage still fails closed.
...
- Latest pushed behavior checkpoint at this snapshot: `08b53ca Recover Prometheus numeric for loops`; latest beta/scheduler checkpoints are `b4ad0c8 Fix beta numeric loop register flow` and `ecc6075 Move numeric VM transitions to leaf tail`. Beta-CF supports compiler-proven numeric `for` loops while unrelated/general cycles still fail closed. Terminal compiler return payloads, nested-child return safety, captured-cell lexical ordering, ambiguous assignments, unresolved captures/cells, and multi-state cell-owner safeguards remain enforced.
- Indexed/member effect-write support from `f16b244` remains active and is consumed normally by beta upvalue recovery/CF.
- Latest fixture checkpoint is `9474c28 Add numeric for-loop sample 24`; latest numeric-for CF checkpoint is `08b53ca Recover Prometheus numeric for loops`; tracked samples 1-24 remain paired.
