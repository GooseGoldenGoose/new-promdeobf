...
3. Ordinary register-value epochs now exist as analysis-only storage/value candidates. Next, add only compiler-backed ownership anchors needed to relate independent same-register assignments when source-binding continuity is provable; never merge merely from register reuse, adjacency, or a future cleanup. Current sample 7 remains the focused conditional-shadowing/early-return regression. Do not promote an epoch into a source local until function ownership, provenance, lifetime, cleanup/handoff, and binding-continuity proof are unique.
   - Untracked `sample/spacial.txt` full-pipeline probe: normal Step 3 succeeds (219/227 reachable dispatcher leaves; 8 dead leaves pruned; 48 functions), beta register versioning succeeds with 0 skipped assignments, but beta-CF currently fails in closure entry 61. The original states 61-63 form a short-circuit/control-join shape: state 61 computes one condition and either reaches control-only state 62 or state 63; state 63 computes the fallback condition then jumps to 62; state 62 branches on the merged `ReturnVal` condition to state 64 or continuation 65. `forwardControlOnlyJoinBranches` removes state 62 and rewrites this to overlapping edges `61 -> {64,63}` and `63 -> {64,65}`. State 64 is then a shared true arm that also reaches 65, so the current acyclic emitter attempts to emit state 64 from two branch paths and rejects with `State 64 would be emitted more than once`. Disabling control-join forwarding exposes the underlying reason it exists: state 62 reads `ReturnVal` with no single proven beta version at the join. This is an acyclic short-circuit/shared-arm structuring gap, not a VM-state, upvalue, or loop-recovery failure; fix must recover that merged boolean/control region structurally rather than duplicate state 64 or invent an unsafe phi.
   - Tracked `sample/47.source.lua` / `sample/47.txt` is the user-supplied anti-tamper/control-flow stress fixture. The two pasted generator-template fragments (`tostring(math.random(...))` / `RandomStrings.randomString()`) were materialized into concrete numeric/string literals so the tracked source is valid standalone Lua; no deobfuscator rule depends on those values. Required source -> local WeAreDevs Medium -> formatter flow succeeds (formatter reports its existing two table-rewrite warnings). Normal deobfuscation resolves 49/57 dispatcher leaves, pruning 8 dead leaves across 8 VM functions; beta versions 327 assignments with 0 skips. Beta-CF fails closed at closure entry 32 with `State 35 would be emitted more than once`. The blocker is the same shared short-circuit/control-join shape proven by `sample/spacial.txt`: `32 -> {33,34}`, `34 -> 33`, then control-only `33 -> {35,36}`. Forwarding state 33 creates overlapping shared true arm 35; current acyclic structuring cannot emit it once without reconstructing the merged boolean region.   - `output/47.beta.lua` raw `ReturnVal` reads are now understood: beta correctly versions nonterminal writes as `r_v4_*`, but at joins where multiple distinct `ReturnVal` definitions reach the same state it deliberately leaves the physical name unresolved because `uniqueVersionMap` only rewrites a read when exactly one incoming version is proven. Confirmed joins include state 3 (`pcall(...) and pcallIntact2`), state 6 (`table and table.unpack or unpack`), state 33 (the `random(...) == 1 or i == n` blocker), plus states 16 and 38. Terminal `ReturnVal = {...}; state=nil` writes and final `return unpack(ReturnVal)` are intentionally preserved. The physical register name `ReturnVal` is therefore semantically misleading for its nonterminal scratch/merge epochs; future CF/value recovery should structurally reconstruct the merged boolean/value region (or otherwise model the merge) rather than arbitrarily select an `r_v4_*`, duplicate shared arms, or treat every physical `ReturnVal` use as a source return value.
...
- Sample 47 is the tracked anti-tamper/shared-short-circuit regression: normal + beta succeed, beta-CF fails closed on shared arm state 35; this is the same confirmed acyclic short-circuit/control-join gap as untracked `sample/spacial.txt`.
