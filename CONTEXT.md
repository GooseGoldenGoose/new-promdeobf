...
- Large untracked `sample\5.txt` validation: normal pipeline succeeds through Step 2, but Step 3 intentionally does not apply because only 810/938 dispatcher leaves are reachable under current terminator recognition (`collision=false`, 0 unresolved reached blocks). Recovered reached terminators are 341 jumps, 313 branches, and 156 currently-classified stops. Of those stops, 150 are normal generated `_env[<string>]` stop sentinels, while 6 are `state or <register>` forms; canonical Prometheus emits this shape for some loop/control-flow transitions, so these are the leading candidate for the 128 missing states. Do not weaken normalization safety; recover this terminator structurally/data-flow first.
- Canonical compiler confirmation for sample 5: setPos(scope, nil) deliberately emits state = _env[randomString(12..14)] instead of literal nil (compiler comment retains --Ast.NilExpression()); this is used on implicit function fall-through and explicit return, and while state do exits when that random environment lookup is nil. The random key itself has no source semantic meaning.
- Canonical numeric-for confirmation: Prometheus builds the loop condition branch in two POS writes using temporaries: load innerBlock.id; state = condition and innerId; load finalBlock.id; state = state or finalId. Sample 5 has six exact four-line forms of this pattern, e.g. W = 2997284; state = i and W; i = 1274385; state = state or i, which is semantically state = i and 2997284 or 1274385. These must be recovered as two-way branches, not stops.
