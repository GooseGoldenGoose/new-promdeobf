...
- The scheduler now compacts a whole producer -> consumer gap instead of only bubbling one consumer left. For each consumer it finds the nearest prior write for each register read, then considers pure filler assignments between the earliest direct producer and the consumer. A filler moves to after the consumer only if it can cross every intervening statement as a sequence of adjacent RAW/WAR/WAW-safe swaps. Transitive producers naturally block movement when crossing their dependent statement would create a hazard. Each original statement is scheduled at most once, preventing movable loads from oscillating around each other.
...
- Sample 8 exposes the improvement clearly: `D = "keep1"` is placed next to `i = l(D, e)`, and `F = "inner"` is placed next to `B = G(F, D)`, while environment lookups/calls retain effectful ordering and the proven cleanup/ownership-handoff writes remain dependency ordered.
