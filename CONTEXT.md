...
Fresh CF now supports proven simple `if` and `if/else` diamonds. One-sided form: one branch contains statement-level discarded-call effects and the other skips directly to the join. Two-sided form: both true/false branch regions contain proven statement effects and converge to the same unique join. Direct conditions and cleanup-backed source-local conditions are supported. The matcher runs only after existing logical/TESTSET recovery, records the exact effect depth at each branch, rejects logical result-carrier merges, nested conditional-effect markers, live path-dependent values at the join, and other ambiguous shapes. General `elseif`, nested statement regions, and arbitrary branch-local declarations/assignments remain fail-closed.
...
9. try proven simple `if` / `if/else` recovery
...
- general `elseif` and nested/arbitrary `if` regions beyond the proven simple `if` / `if/else` diamond shapes
- general `while`
- general `repeat`
- numeric `for`
- generic `for`
...
Limited compiler short-circuit multi-state logic and the proven simple `if` / `if/else` shapes are supported separately, as documented above.
...
- simple `if` and `if/else` recovery supports direct conditions and cleanup-backed local conditions; one-sided and two-sided statement-effect regions are distinguished from TESTSET by successor/join structure and by refusing logical result-carrier merges. The direct/local `if/else` user fixtures plus the existing `or`/TESTSET fixture passed exact runtime parity and 5/5 randomized Medium recompiles; focused tests keep TESTSET in `fresh-multistate-logical`.
...
Those remain separate future features. Limited compiler-generated short-circuit logical CFGs and the proven simple `if` / `if/else` shapes are supported; neither may be generalized into arbitrary CFG guessing.
