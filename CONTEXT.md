...
- Current tracked `sample/7.source.lua` now puts `return 123` inside the inner `do ... end` immediately after `print(b)`. Therefore the later outer `print(b)`, nested `w()` declaration/use, and its runtime capture are unreachable. Source, formatted Prometheus Medium fixture, and deobfuscated output all print `1`, `2` and return chunk value `123` with exit 0.
- After correct formatter normalization, sample 7 has 2 dispatcher leaves but only the root state is reachable. Step 3 safely normalizes the root to state `1` and prunes 1 compiler-emitted unreachable dispatcher leaf belonging to code after the early return. This is a useful dead-code/reachability regression; it no longer proves post-block restoration of the outer shadowed `b` or live nested-closure capture behavior.
- Current reachable root data flow still shows separate source-value lifetimes: the outer `b = 3` is loaded into an ordinary VM register, source `a = 1` is stored through a local upvalue cell because the source is lexically captured by the later function, and inner `b = 2` is a separate temporary used by the second print before the early stop. Register spellings are randomized fixture evidence only and must never become implementation assumptions.
...
- Shared-cell identity is now explicit: sample 3's two closures resolve to the same local cell and all 5 cell accesses resolve to it; sample 4's three closures resolve to the same `total` cell and all 17 accesses resolve; sample 6 resolves the root->outer and outer->inner captures to two different cells with all 14 accesses resolved. Current sample 7 has only the reachable root function after its early return: 1 local cell, 0 reachable capture slots, and 2/2 cell accesses resolve.
...
- Definition-site liveness is now computed backward over each recovered function CFG using the reaching-definition identities. Each definition records use counts, live-in/live-out blocks, cross-block lifetime, and loop-carried status. Alternative definitions are placed in a conservative join component only when they co-reach the same identifier use; sequential register reuse is not merged. Sample 4 has one such join component and one loop-carried lifetime; sample 6 has cross-block lifetimes; current sample 7 has 17 definitions, 12/12 uniquely resolved uses, and no joins/cross-block/loop-carried lifetimes; sample 5 converges with 177 join components, 695 cross-block lifetimes, and 61 loop-carried lifetimes.
- Captured source-binding candidates now exist as a separate proof class keyed by static `allocUpvalue()` definition identity. Every candidate summarizes its owning function, resolved `upvalueValues` reads/writes, capture consumers, and transitive relays. Capture edges themselves carry final `resolvedCellId` when provable. Samples 3/4/6 have complete reachable capture graphs. Current sample 7 has one complete local-cell candidate but no reachable capture consumer because the closure-creation path is dead after the source `return 123`. Sample 5 remains fail-closed with 92 unresolved capture edges plus 182 unresolved cell accesses, so its 112 candidates are marked provenance-incomplete and are not eligible for source rewriting.
...
3. For ordinary locals, build conservative register-value epochs from definition/use/liveness connectivity without merging sequential unrelated values. Current sample 7 is now an early-return/dead-code scope regression; use a reachable shadowing fixture when proving post-scope restoration. Only promote a source local when function ownership, provenance, and lifetime proof are unique.
