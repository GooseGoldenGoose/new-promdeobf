...
- Beta-CF currently fails closed. After all existing loop collapses, exactly one residual SCC remains: states 32 <-> 33. State 32 is the original repeat body `task.wait(5)`, now containing EncryptStrings decrypt-helper/upvalue lookup operations; state 33 is the original `game:IsLoaded()` condition with true exit / false backedge. The loop topology is valid, but the repeat duplicate-condition/preheader matcher no longer recognizes the compiler-added pre-condition region because EncryptStrings inserts extra helper/cell setup around it. This is the current structural blocker; do not treat it as a generic irreducible loop.
