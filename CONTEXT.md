...
Latest implementation checkpoint is `04114c3 Cache beta parser fragments` on `main`, pushed to `origin/main`; latest fixture checkpoint is `4d100bd Add while-loop sample 27`. Numeric-for CF behavior remains at `5f7c7f4 Recover numeric for loops with branched bodies`: sequential, nested, and branched-body numeric loops are supported. Sample 27 now provides the focused general-`while` backedge regression; normal and beta succeed, while beta-CF intentionally still fails closed because general while-loop structuring is not implemented. All tracked fixtures `sample/1` through `sample/27` remain paired.
...
- Every tracked `sample/N.txt` must have a matching tracked `sample/N.source.lua`. Current paired coverage is samples 1-27.
...
- Current paired fixture coverage is samples 1-27. New sample 5 schedules 390 blocks with 221,956 dependency-safe swaps, 217 unread sinks, and a structurally proven 23-slot overflow bank. Sample 10 remains the natural overflow-boundary fixture with 19 slots.
...
- Focused regression `tools/test-semantic-names.js` covers `InitialArgs`, fixed closure `argN`, `gcProxy`, `closure`, `captureIndex`, `upvalueId`, `proxy`, `proxyMetatable`, and `RegisterOverflow`. Current paired fixtures 1-27 remain parse/runtime-valid where executable.
...
5. Extend beta CFG beyond the proven numeric-`for` handler to `while`, `repeat`, and other/general backedge shapes using dominators/natural-loop proof; sample 4 remains the focused if/while/repeat regression and sample 6 the nested-function/if/while regression.
...
- Latest pushed implementation checkpoint at this snapshot: `04114c3 Cache beta parser fragments`; latest normal-pipeline optimization is `11a5629 Optimize deobfuscation pipeline passes`; latest fixture checkpoint is `4d100bd Add while-loop sample 27`. Numeric-for CF behavior checkpoint remains `5f7c7f4 Recover numeric for loops with branched bodies`. Beta-CF supports compiler-proven numeric `for` loops while general `while`/other unmatched cycles still fail closed. Terminal compiler return payloads, nested-child return safety, captured-cell lexical ordering, ambiguous assignments, unresolved captures/cells, and multi-state cell-owner safeguards remain enforced.
- Indexed/member effect-write support from `f16b244` remains active and is consumed normally by beta upvalue recovery/CF.
- Latest fixture checkpoint is `4d100bd Add while-loop sample 27`; latest numeric-for CF checkpoint is `5f7c7f4 Recover numeric for loops with branched bodies`; tracked samples 1-27 remain paired.
...
- Sample 27: focused general `while` regression from `while math.random(1,2) == 1 do print(27) end`. It follows readable source -> local WeAreDevs Medium -> formatter. Normal resolves 4/4 states in one VM function with 19 reaching definitions, no captures/cells, and no unresolved accesses. Beta reports 4 state blocks / 14 versioned assignments / 5 preserved final state/return writes / 0 ordered effect writes / 0 cross-state versions / 0 skipped assignments and prunes 4 physical register declarations. Source, raw Medium, formatted, normal, and beta output match exactly across deterministic seeds 1-20. Beta-CF currently fails closed with `Beta CF acyclic stage detected a loop/backedge; loop structuring is not implemented yet`; this is now the focused fixture for implementing general Prometheus `while` recovery.
