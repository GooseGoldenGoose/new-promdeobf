...
- Every tracked `sample/N.txt` must have a matching tracked `sample/N.source.lua`. Current paired coverage is samples 1-23.
...
- Current paired fixture coverage is samples 1-23. New sample 5 schedules 390 blocks with 221,956 dependency-safe swaps, 217 unread sinks, and a structurally proven 23-slot overflow bank. Sample 10 remains the natural overflow-boundary fixture with 19 slots.
...
5. Next beta-CF correctness target exposed after indexed/member support: terminal-return canonicalization for nested single-state closures. Sample 20 now reaches closure entry 36 and fails closed because a proven compiler return payload is separated from `state = nil` by beta operations that are not yet proven safe to cross. Do not embed that child until the return can be lowered structurally; investigate the original compiler/scheduler ordering and add a proof rather than weakening the guard.
6. Extend beta CFG structuring to loops/backedges using dominators/backedge loop discovery before emitting `while`/`repeat`; sample 4 remains the focused if/while/repeat regression, sample 6 the nested-function/if/while regression, and sample 7 the branch/early-return + capture regression.
7. Keep POS-register temporary reuse distinct from true block terminators; extend Prometheus-specific recognition only from evidence in the local WeAreDevs compiler unless the user explicitly requests the public/canonical implementation.
8. Keep every pass structural/generalized, reparse transformed Lua, runtime-check executable fixtures, and update/commit/push `CONTEXT.md` after meaningful work.
9. If the current parser becomes a correctness blocker, evaluate Rust Moonlight instead of parser-specific hacks.
...
- Latest pushed beta behavior checkpoint at this snapshot: `f16b244 Support beta indexed effect writes`; beta register versioning now emits structurally simple indexed/member stores as ordered `effect-write` operations with rewritten reaching-version reads, and CF preserves them as indivisible side effects. Non-root closure regions additionally require fully lowered terminal returns before nested-function embedding. Captured-cell recovery remains active; samples 18/19 still solve and execute exactly. Ambiguous assignments, unresolved captures/cells, multi-state cell owners, unlowered child returns, and loops/backedges still fail closed.
...
- Latest fixture checkpoint: `95dda19 Add reduced language basics sample 21`.
- Prior sample-21 context checkpoint: `10e9233 Document reduced sample 21 results`; current beta behavior is `f16b244`.
...
- Sample 20: paired broad Luau language/library basics stress fixture supplied from the Luau test suite and refreshed by the user to avoid the earlier compiler-triggering source form. It covers global/vararg recursion, fixed-arg behavior, direct and propagated upvalues, captured function arguments, local-slot aliasing, mutable locals/upvalues, `self` capture, if/else, and arithmetic. It follows the required source -> local WeAreDevs Medium -> formatter flow. Main resolves 47/48 dispatcher leaves (1 dead leaf pruned) across 35 VM functions, with 391 reaching definitions, 16 capture slots, 9 local cells, 2 shared cells, and 36/36 upvalue accesses resolved. Runtime parity is exact across readable source, raw local Medium output, formatted fixture, normal deobfuscated output, and beta-only output: each prints `testing language/library basics` then `PASS`. Beta register versioning now analyzes 47 blocks / 308 versioned assignments / 21 ordered indexed/member effect writes / 6 cross-state versions / 0 skipped assignments. Upvalue recovery remains safe/applied. Indexed/member writes are no longer a blocker; beta CF advances through all of them but now fails closed at closure entry 36 because that child region's terminal VM return cannot yet be fully lowered. A safety guard rejects nested-function embedding in this case, preventing the nil-return semantic corruption observed during the exploratory pre-guard run. This is now a return-lowering/canonicalization boundary, not an indexed-write or upvalue-resolution boundary.
...
- Sample 23: paired reduced language-basics fixture covering recursive global vararg `concat`, fixed-argument `foo`, missing/extra argument behavior, and two immediately-invoked local-function branches (`local a = 1` truthy mutation and uninitialized `local a` false path). It follows readable source -> local WeAreDevs Medium -> formatter. Main resolves 11/12 dispatcher leaves (1 dead leaf pruned) across 5 VM functions, with 100 reaching definitions, 2 joins, 6 cross-block lifetimes, no capture slots/cells, and 0/0 upvalue accesses. Beta reports 11 blocks / 83 versioned assignments / 1 ordered indexed/member effect write / 4 cross-state versions / 0 skipped assignments, with 42 register epochs and 1/1 cleanup attached. Beta CF succeeds in `closure-regions` mode with 11 states, 5 closure regions, 4 inlined closure factories, 3 branches, 2 joins, 1 guard-return branch, and all 6 terminal returns lowered. Source, raw Medium, formatted fixture, normal output, beta output, and beta-CF output all execute identically and print only `testing language/library basics`.
