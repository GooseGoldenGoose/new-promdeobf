...
- Large untracked sample 5 still resolves the same closed 930/938 dispatcher graph with 8 dead leaves and 116 reachable functions. Reachability-driven closure discovery does not change that proven state count. Current scheduling changes 571 blocks through 67,625 dependency-safe adjacent crossings and sinks 454 unread pure writes; no safety-validator rejection is reported. The high swap count is a count of equivalent adjacent crossings, not independent semantic rewrites.
- Full post-scheduler batch validation on 2026-08-23 now covers every current `sample/*.txt` fixture 1 through 9, including untracked stress sample 5. All nine exit 0 and reparse. Runtime parity matches samples 1/2/3/4/6/8/9 and both deterministic sample-7 seeds; sample 5 remains structural/stress validation only. Both focused scheduler and state-reachability regressions pass.
