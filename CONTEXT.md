...
Latest implementation checkpoint is `a265716 Recover while loops with guard breaks` on `main`, pushed to `origin/main`; latest fixture checkpoint is `b832da0 Add mixed loop sample 28`. Beta-CF supports compiler-proven numeric `for` loops plus pre-test `while` natural loops emitted in guard-break form. Sample 28 proves mixed nesting across `for` inside `while`, `while` inside `for`, multiple `if/else` branches, `do` blocks, and a separate nested function containing its own mixed loops. All tracked fixtures `sample/1` through `sample/28` remain paired.
...
- Current paired fixture coverage is samples 1-28. New sample 5 schedules 390 blocks with 221,956 dependency-safe swaps, 217 unread sinks, and a structurally proven 23-slot overflow bank. Sample 10 remains the natural overflow-boundary fixture with 19 slots.
...
- Focused regression `tools/test-semantic-names.js` covers `InitialArgs`, fixed closure `argN`, `gcProxy`, `closure`, `captureIndex`, `upvalueId`, `proxy`, `proxyMetatable`, and `RegisterOverflow`. Current paired fixtures 1-28 remain parse/runtime-valid where executable.
...
- Latest pushed implementation checkpoint at this snapshot: `a265716 Recover while loops with guard breaks`; latest normal-pipeline optimization is `11a5629 Optimize deobfuscation pipeline passes`; latest fixture checkpoint is `b832da0 Add mixed loop sample 28`. Beta-CF supports compiler-proven numeric `for` loops and pre-test `while` natural loops. `while` conditions remain as beta operations inside `while true`, followed by `if not (<condition>) then break end`, preserving per-iteration evaluation. Unmatched cycles/repeat loops still fail closed. Terminal compiler return payloads, nested-child return safety, captured-cell lexical ordering, ambiguous assignments, unresolved captures/cells, and multi-state cell-owner safeguards remain enforced.
- Indexed/member effect-write support from `f16b244` remains active and is consumed normally by beta upvalue recovery/CF.
- Latest fixture checkpoint is `b832da0 Add mixed loop sample 28`; latest numeric-for CF checkpoint is `5f7c7f4 Recover numeric for loops with branched bodies`; tracked samples 1-28 remain paired.
