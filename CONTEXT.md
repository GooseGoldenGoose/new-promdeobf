...
- `passes/vm-register-scheduler.js` now runs after safe Step-3 normalization. Its purpose is readability: pull a statement toward the in-block register definition(s) it actually consumes, so unrelated temporary loads do not separate a producer/use chain. It never uses fixture register names or state IDs.
- Scheduling is structural and local to normalized dispatcher leaves. Comments are barriers; blocks with non-whitespace source gaps are left unchanged. A statement is considered for movement only when it reads a register defined earlier in the same block. It may move left only across a single-register assignment whose RHS is a primitive literal or identifier copy; assignments to `state`, `args`, `upvalues`, or `gcProxy` are never treated as movable fillers.
- Every adjacent swap checks register RAW, WAR, and WAW dependencies. Any dependency blocks movement, so effectful operations keep their relative order and a consumer can cross only independent pure register loads. Table constructors, calls, function literals, indexed writes, arithmetic expressions, and other potentially effectful/value-identity-sensitive RHS forms are not movable fillers.
- Focused sample-1 result is now exactly the requested chain: `R = "print"; state = _env[R]; q = "AD"; R = state(q)` while unrelated `V = args` is left after that chain. Sample 1 required 3 dependency-safe swaps.
- Tracked regression scheduling counts after the new pass: sample 1 -> 1 block / 3 swaps; sample 2 -> no change; sample 3 -> 1 block / 6 swaps; sample 4 -> 1 block / 2 swaps; sample 6 -> 2 blocks / 9 swaps; sample 7 -> 2 blocks / 3 swaps. All outputs reparse and LuaJIT runtime parity matches for samples 1, 2, 3, 4, 6, and both deterministic sample-7 branches.
- Large untracked sample 5 also completes: the same 930-state closed graph is preserved, 8 unreachable leaves remain pruned, and scheduling performs 1,207 dependency-safe swaps across 215 blocks without changing state normalization or Step-4 analysis counts.
