...
- Adjacent producer temps may also move into a following direct-local call argument when the outer call base is a stable lexical local and every earlier argument is only a literal/stable lexical snapshot. Calls are parenthesized when substituted so a local that captured one return value cannot accidentally expand multiple returns in final-argument position. Nested-closure writers of the outer call base or earlier local arguments are hard barriers. This safely handles repeat/compiler chains such as `local n = #t; local i = random(1,n)` and `local i = random(...); local v = remove(t,i)` without reordering effectful expressions.
...
- Pre-fold direct `x = nil` lifetime cleanup handles nested lexical blocks that end in a direct function `return` and repeat bodies whose tail condition does not observe the local. It still requires a prior declaration in that exact block, no nested closure capture/upvalue use, and no later read/redeclare; ordinary non-terminal nested branches still fail closed.
