...
- `main.js` exposes compact reaching-definition, lifetime, and cell-access counts. Final source output remains exactly the Step 3 normalized source for now. Reparse still occurs after the final emitted source, and LuaJIT runtime parity was reconfirmed for samples 3, 4, 6, and 7 after both Step-4 extensions.
...
2. Build conservative source-binding candidates by relating definition lifetimes to resolved cell accesses, closure captures, function ownership, and local-vs-environment provenance. Captured-cell bindings and ordinary register-value bindings must remain separate classes.
3. For ordinary locals, use lifetime/use connectivity to detect register-reuse epochs without merging sequential unrelated values. Sample 7 remains the primary shadowing regression; only promote a source local when ownership and lifetime proof are unique.
