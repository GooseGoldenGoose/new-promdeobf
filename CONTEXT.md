...
5. Immediate requested beta-CF work: add structural support for ordered indexed/member assignment statements such as `base[index] = value` (and member equivalents) instead of classifying every such statement as unsupported. Current concrete blockers are sample 21 root `_env[key] = closure`, emitted from source global `concat = function(...)`, and sample 20 table/method member assignment. Preserve exact statement order/evaluation, never duplicate/drop/move the write across effects, branches, or returns, reuse the already-proven beta read rewrites for base/index/RHS, and fail closed on ambiguous assignment shapes. Add focused synthetic regressions, then rerun sample 20 and sample 21 and runtime-check any generated beta-CF outputs.
6. After indexed/member effect-statement support, extend beta CFG structuring to loops/backedges using dominators/backedge loop discovery before emitting `while`/`repeat`; sample 4 remains the focused if/while/repeat regression, sample 6 the nested-function/if/while regression, and sample 7 the branch/early-return + capture regression.
...
- Latest pushed project/context checkpoint before this handoff update: `10e9233 Document reduced sample 21 results`.
...
- Sample 20: paired broad Luau language/library basics stress fixture supplied from the Luau test suite and refreshed by the user to avoid the earlier compiler-triggering source form. It covers global/vararg recursion, fixed-arg behavior, direct and propagated upvalues, captured function arguments, local-slot aliasing, mutable locals/upvalues, `self` capture, if/else, and arithmetic. It follows the required source -> local WeAreDevs Medium -> formatter flow. Main resolves 47/48 dispatcher leaves (1 dead leaf pruned) across 35 VM functions, with 391 reaching definitions, 16 capture slots, 9 local cells, 2 shared cells, and 36/36 upvalue accesses resolved. Runtime parity is now exact across readable source, raw local Medium output, formatted fixture, normal deobfuscated output, and beta-only output: each prints `testing language/library basics` then `PASS`. Beta register versioning analyzes 47 blocks / 308 versioned assignments / 6 cross-state versions / 21 skipped assignments. Beta upvalue recovery reports safe/applied on the refreshed graph. Beta CF still fails closed, currently at closure entry 12, because that single-state region contains an unsupported table/member indexed assignment; no `output/20.beta.cf.lua` is emitted. This is a CF operation-coverage limitation, not an upvalue-resolution failure or runtime-parity failure.
- Sample 21: paired reduced language-basics fixture supplied by the user. It keeps global vararg `concat`, fixed-argument `foo`, four if/else assertions, four arithmetic assertions, and a table literal with a closure-valued `tad` field. Required source -> local WeAreDevs Medium -> formatter flow succeeds. Main resolves 24/25 dispatcher leaves (1 dead leaf pruned) across 12 VM functions, with 211 reaching definitions, no capture slots/cells, and 0/0 upvalue accesses. Source, raw Medium, formatted fixture, normal output, and beta-only output all execute successfully and print `testing language/library basics` then `PASS` plus the runtime table identity. Beta analyzes 24 blocks / 174 versioned assignments / 6 cross-state versions / 1 skipped assignment. Beta CF fails closed in root closure entry 1 because source global assignment `concat = function(...)` is VM-emitted as `_env[key] = closure`, an indexed environment write currently classified as unsupported; the later table constructor `g = { hee=1, kuy=2, tad=function() ... end }` itself is represented as a supported value expression and is not the blocker. No `output/21.beta.cf.lua` is emitted.
- Pending user-requested next implementation: teach beta CF to preserve structurally safe indexed/member writes as ordered effect statements. This is not implemented yet. The first acceptance target is sample 21 so `_env[key] = closure` survives solver restructuring and `output/21.beta.cf.lua` can be emitted if no later unsupported region remains; sample 20 is the second real regression.
