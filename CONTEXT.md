...
Latest beta behavior checkpoint is `f6345a8 Structure acyclic beta control flow` on `main`, pushed to `origin/main`. Beta CF now structures a closed, single-entry, acyclic multi-state CFG instead of requiring one state. It parses only proven normalized transitions, removes direct state jumps, converts `condition and trueState or falseState` into source `if`, computes post-dominators to identify real branch joins, emits guard-return form when both arms terminate, and lowers each proven terminal `ReturnVal = { ... }; state = nil` pair to Lua `return`. Branch conditions must already be precomputed beta-register identifiers; loops/backedges, multiple entries, unsupported operations, unsafe terminal returns, non-reducible reuse, and beta locals that would escape a new Lua branch scope all fail closed. Beta remains analysis/presentation-only and source lexical-scope reconstruction is still NOT implemented. All tracked fixtures `sample/1` through `sample/15` remain paired.
...
- Current paired fixture coverage is samples 1-15. New sample 5 schedules 390 blocks with 221,956 dependency-safe swaps, 217 unread sinks, and a structurally proven 23-slot overflow bank. Sample 10 remains the natural overflow-boundary fixture with 19 slots.
...
- Beta control-flow presentation lives in `passes/beta-control-flow.js` with CLI `node tools/beta-control-flow.js <output.lua> [output.beta.cf.lua]`. Single-state terminal CFGs keep the original flattening path. Multi-state support now covers closed, single-entry, acyclic CFGs: direct jumps are removed into fallthrough; two-way normalized state transitions are parsed structurally and require a precomputed beta-register condition; post-dominators identify branch joins; one-empty-arm joins become `if ... then ... end`; two non-empty arms become `if/else`; and branches whose immediate post-dominator is function exit use an early-return guard followed by the other arm as fallthrough. Every terminal block must lower to a proven compiler return. The solver rejects loops/backedges, multiple entries, missing targets, unsupported operations, non-unique/non-reducible regions, and branch-local beta declarations that would become out of scope. Headers remain a structural `_env` provider when required plus `local args = { ... }`.
...
- Focused regression `tools/test-semantic-names.js` covers `InitialArgs`, fixed closure `argN`, `gcProxy`, `closure`, `captureIndex`, `upvalueId`, `proxy`, `proxyMetatable`, and `RegisterOverflow`. Current paired fixtures 1-15 remain parse/runtime-valid where executable.
...
- Latest pushed beta behavior checkpoint at this snapshot: `f6345a8 Structure acyclic beta control flow`; beta CF now structures closed single-entry acyclic multi-state graphs with post-dominator joins and guard-return branches while loops/backedges still fail closed.
...
- Sample 15: paired `if`/`elseif` + early-return fixture with mutable `g`. It follows the required source -> local WeAreDevs Medium -> formatter workflow. Main resolves 9/9 states in one function, and readable source, formatter-normalized obfuscation, and normal deobfuscated output match exactly for deterministic seeds 1-20. This fixture currently exposes an acyclic-beta-CF reducibility gap: `tools/beta-control-flow.js output/15.lua` fails closed with `State 8 would be emitted more than once; CFG is not structurally reducible by the acyclic stage`. Keep sample 15 as the focused regression for shared `elseif` regions; do not hardcode its state IDs.
