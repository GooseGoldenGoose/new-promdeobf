...
## Current Handoff Snapshot â€” 2026-08-30 after Step 47
...
9. Stop after the completed step and end exactly: `Done for this turn â€” you can prompt now.`
...
## Performance plan Step 11 - final benchmark/profile (2026-08-31)
- Roadmap complete. No solver code changed in this step.
- Final clean warm `spacial6` rerun: 40.290 s / 41.260 s / 41.264 s; median 41.260 s. All three runs recovered 3799 states / 554 closures, reparsed, had zero `RegisterOverflow[`, `createClosure`, `upvalueValues[`, or `ReturnVal =` scaffold, and matched the frozen final byte-for-byte (SHA-256 `14d488e79025f385dc79038cf557a5f74a5390e9924647772d9086bf4a5d4145`).
- The accepted best checkpoint remains Step 8: 40.101 s median (40.101 / 39.891 / 40.487), because Steps 9-10 made no code changes and the final rerun variation is runtime/GC noise. Versus the original 76.616 s baseline, the accepted median is about 47.7% faster. It did not reach the <40 s first target reliably; historical ~9-11 s remains unrecovered.
- Canonical combined gate PASS: 43 focused suites + 66/66 samples; measured wall time 9.160 s for that gate on this final run.
- One end-to-end instrumented `spacial6` run counted 1,342,031 calls through `parser/luaparse.parse` and `process.resourceUsage().maxRSS = 2,954,004 KiB` (~2.82 GiB). This parser-call count uses a broad end-to-end monkey-patch and should not be compared directly with older narrower parser-call profiles unless measurement scope is matched.
...
## Stress-fixture correctness fixes (2026-08-31)
...
## Temp compiler audit continuation checkpoint (2026-08-31)
- User requested compiler-wide Prometheus temp recovery rather than sample-by-sample cleanup. Local compiler audit confirms real source locals are VAR_REGISTER lifetimes with scope-end cleanup, while compiler transport temps are allocRegister(false)/immediate-use storage. Recovery must combine compiler emission shape + lifetime/source ownership + exact use topology; absence of a kill alone is not enough because source locals can adopt expression registers.
- `finalizeBetaDeadRegisterClears` now derives `compilerSourceLifetimeProven` from authoritative beta epoch `kill` events and propagates that marker to surviving same-name operations before removing cleanup statements. This protects genuine source aliases/locals from later temp folding.
- In-progress PRE-CF extensions:
  - generalized literal/adjacent return-expression transport, preserving source-lifetime-marked producers;
  - mixed multi-return pack recovery where every pack read is a static numeric slot and slots are contiguous 1..N; one missing extracted slot may reuse the pack binding as the scalar destination; dynamic/whole-pack/sparse/duplicate/captured/ambiguous cases refuse;
  - RETURN_ALL table-tail bridge recovery for exact compiler `{ call(...) } -> { unpack(pack) }` transport while preserving the real source table destination;
  - direct vararg compiler signature recovery for exact `select(fixed+1, unpack(args))` under `createClosure`, rewritten to native `...`; wrong offsets/non-vararg factories refuse.
- In-progress post-CF extension: `recoverStructuredCompilerValueTemps` can fold proven compiler temp chains into raw multi-call-write / raw scalar consumers only when every consumed producer is compiler-owned and not `compilerSourceLifetimeProven`. `args[N]` parameter-load operations are a hard source-binding barrier. An earlier raw-return fold was removed after regression testing because it could erase source return locals.
- Stress fixture current verified semantic shape: 72 states / 8 closures. It now emits direct `return (math.random(1, 5)), "hello"`, vararg closure `return ...`, direct multi-return assignment, and removes the inner compiler RETURN_ALL bridge while preserving deliberate real source constructs such as `local alias = print` and `local packed = { returnsMany(...) }`.
- Focused/new tests currently passing include closure signatures, literal return temps, multi-return temps, RETURN_ALL temps, beta-control-flow, and the existing temp suites. Full combined regression currently PASS: **44 focused suites + 66/66 canonical samples**.
- Performance regression was resolved before finalization. The broad post-CF raw scalar/multi-call consumer path was removed after A/B profiling: it raised `spacial6` normal->PRE-CF->CF from ~60 s to ~84 s and also made one anonymous closure presentation worse. Its useful cases were moved into the existing batched PRE-CF call-setup pass instead.
- `finalizePreCfCallSetupChains` now also handles argument-only literal setup for proven `multi-call-write` consumers and for an exact single-call table constructor `{ f(...) }`. The call base may remain a real source binding; only contiguous one-def/one-use literal compiler setup temps are consumed. This recovers direct multi-return argument temps and table-tail call argument temps without generic inlining.
- `finalizePreCfLiteralReturnTemps` now batches independent terminal return payloads per proof snapshot. On `spacial6`, 22 folds improved from ~7.4 s to ~2.1 s while preserving the same source-lifetime and adjacency/effect rules.
- `recoverStructuredCompilerValueTemps.recoverSuffix` now tries only transitive producer-dependency start positions instead of every possible prefix start. Focused post-value and main beta-CF regressions pass. `spacial6` normal->PRE-CF->CF benchmark improved to **39.5 s**, back under the historical ~40 s target.
- Final production `spacial6` run completed in ~42.5 s including normal/formatter overhead: **3799 states / 554 closures**, reparse PASS, 0 `RegisterOverflow[`, 0 `createClosure*`, 0 `upvalueValues[`, and 0 `ReturnVal =` scaffold. Full combined regression PASS: **44 focused suites + 66/66 canonical samples**.
- Stress fixture remains **72 states / 8 closures** and now keeps the cleaner PRE-CF result: direct multi-return literal arguments, source table `{ returnsMany("A", "B", "C") }` preserved, native `return ...`, direct call-return expressions, and anonymous `pcall(function() ... end)` remains inline. Genuine source alias `local alias = print` remains protected.
- Preserve unrelated tracked dirt in `formater/input.txt`, `main.js`, `opti/spacial6.lua`, plus the pre-existing unrelated dead-closure/dead-scalar batching hunks in `passes/beta-control-flow.js`. When staging this temp-audit feature, exclude those unrelated beta-CF batching hunks exactly as in the prior index-only staging workflow.


## Remaining compiler temp cleanup (2026-08-31)
- User requested cleanup of the remaining compiler transport seen in the 72-state stress fixture, specifically custom-iterator return temps, shared OR-condition carrier temps, and adjacent `local x` + `x = value` presentation. Genuine source aliases such as `folder/items/nested` and the final reassigned `chained` sequence remain intentionally preserved.
- `recoverStructuredCompilerValueTemps` now also folds adjacent compiler-owned scalar shells when the producer is single-def/single-use, not source-lifetime-owned/captured, and the consumer contains no other dynamic operand. Parameter loads from `args[N]` are hard barriers.
- Return-tail transport now peels compiler-owned chains right-to-left into `return` only with physical-epoch provenance or a pure compiler version-copy; source binary return locals refuse. The stress custom iterator now emits `return r_v2_17, u_v1[r_v2_17]` instead of four transport locals.
- Added `recoverStructuredCompilerLogicalCarriers` for the exact compiler OR shape: `cond=lhs; result=cond; if not cond then result=rhs end; if result then ... end` -> `if lhs or rhs then ... end`. Requires physical compiler epochs, matching result epoch, no source ownership/capture, and exact read counts; no call duplication/reordering. Stress OR now emits `if math.random(1, 2) == 1 or os.clock() > 0 then`.
- Added `recoverStructuredAdjacentLocalInitializers`: adjacent `local x` followed by `x = expr` becomes `local x = expr` only when RHS does not read/capture `x` and is not an `args` parameter load. This cleans the loop carrier to `local r_v22_20 = r_v5_1` while preserving closure signature recovery.
- Focused regressions added for return-tail folding/refusal, OR carrier reconstruction, initializer merge + args/self refusal, and numeric-for compiler transport cleanup. Full combined gate PASS: 44 focused suites + 66/66 canonical.
- Real stress fixture PASS: 72 states / 8 closures. Targeted compiler temps `r_v1_21/r_v3_8/r_v4_4/r_v13_11`, `r_v22_5/r_v20_7/r_v15_22` are gone; `local r_v22_20 = r_v5_1` is merged.
- Large `sample/spacial6.txt` production PASS: 3799 states / 554 closures, 46.9 s wall in this run, reparse PASS, zero `RegisterOverflow[`, `createClosure*`, `upvalueValues[`, and `ReturnVal =` scaffold.

## Remaining transport + direct-only multi-return cleanup (2026-08-31)
- Added structural post-CF cleanup for compiler-owned adjacent scalar transport, compiler return-tail transport, exact short-circuit OR carrier epochs, and presentation-only `local x` + immediate `x = rhs` merging. Parameter loads through `args[N]` and self-referential initializers remain hard refusal cases.
- Real source locals/aliases remain preserved; stress `folder/items/nested` and source `chained` assignments are intentionally not treated as compiler temps.
- Stress output now recovers custom iterator return directly as `return r_v2_17, u_v1[r_v2_17]`, OR as `if math.random(1, 2) == 1 or os.clock() > 0 then`, and `local r_v22_20 = r_v5_1`.
- Fixed PRE-CF multi-return gap where a packed call had no extracted locals and every result slot was consumed directly by an effect call, e.g. `{ pcall(fn) }` followed by `print(pack[1], pack[2])`. First missing slot reuses the pack binding; additional slots receive collision-free `<pack>_retN` scalar bindings. All-direct non-effect assignment consumers still fail closed.
- Stress pcall now emits `local r_v19_1, r_v19_1_ret2 = pcall(function() ... end)` followed by `print("PCALL", r_v19_1, r_v19_1_ret2)`.
- Verification: focused multi-return PASS; beta CF focused PASS; combined regression PASS (44 focused suites, 66/66 canonical); stress 72 states / 8 closures; spacial6 3799 states / 554 closures, full production ~45.4s, reparse PASS, zero RegisterOverflow/createClosure/upvalueValues/ReturnVal scaffold.

## Source pack -> final unpack forwarding (2026-08-31)
- Per user reconstruction policy, recovered real source locals of exact shape `local pack = { call(...) }` followed immediately by one final-argument `unpack(pack)` effect-call consumer can now forward the inner call directly into that final argument. This intentionally allows the original call to move from the preceding pack statement into the consumer call.
- `recoverStructuredSourcePackUnpackForwarding` requires `compilerSourceLifetimeProven === true`, one definition/read, no capture, adjacency, an exact single-call table constructor, an effect-call consumer, and `unpack(pack)` as the final argument. Non-source/compiler packs, extra reads/escape, captures, non-final unpack, and non-effect consumers refuse.
- Fresh re-obfuscated stress output now emits `print("PACKED", r_v31_4("A", "B", "C"))` with the real source packed table removed. Focused positive/refusal test PASS; combined regression PASS (44 focused suites, 66/66 canonical). spacial6: 3799 states / 554 closures, ~51.1s full production, project parser reparse PASS, zero RegisterOverflow/createClosure/upvalueValues/ReturnVal scaffold.
