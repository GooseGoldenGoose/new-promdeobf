...
Latest implementation checkpoint is `a265716 Recover while loops with guard breaks` on `main`, pushed to `origin/main`; latest fixture checkpoint is `b70bb2d Add repeat-until sample 29`. Beta-CF supports compiler-proven numeric `for` loops plus pre-test `while` natural loops emitted in guard-break form. Sample 28 proves mixed nesting across `for` inside `while`, `while` inside `for`, multiple `if/else` branches, `do` blocks, and a separate nested function containing its own mixed loops. All tracked fixtures `sample/1` through `sample/29` remain paired.
...
- Every tracked `sample/N.txt` must have a matching tracked `sample/N.source.lua`. Current paired coverage is samples 1-29.
...
- Current paired fixture coverage is samples 1-29. New sample 5 schedules 390 blocks with 221,956 dependency-safe swaps, 217 unread sinks, and a structurally proven 23-slot overflow bank. Sample 10 remains the natural overflow-boundary fixture with 19 slots.
...
- Focused regression `tools/test-semantic-names.js` covers `InitialArgs`, fixed closure `argN`, `gcProxy`, `closure`, `captureIndex`, `upvalueId`, `proxy`, `proxyMetatable`, and `RegisterOverflow`. Current paired fixtures 1-29 remain parse/runtime-valid where executable.
...
5. Extend beta CFG beyond the proven numeric-`for` and pre-test `while` handlers to `repeat`, explicit `break` exits, and other/general backedge shapes using natural-loop proof; sample 4 remains the focused if/while/repeat regression.
...
- Latest pushed implementation checkpoint at this snapshot: `a265716 Recover while loops with guard breaks`; latest normal-pipeline optimization is `11a5629 Optimize deobfuscation pipeline passes`; latest fixture checkpoint is `b70bb2d Add repeat-until sample 29`. Beta-CF supports compiler-proven numeric `for` loops and pre-test `while` natural loops. `while` conditions remain as beta operations inside `while true`, followed by `if not (<condition>) then break end`, preserving per-iteration evaluation. Unmatched cycles/repeat loops still fail closed. Terminal compiler return payloads, nested-child return safety, captured-cell lexical ordering, ambiguous assignments, unresolved captures/cells, and multi-state cell-owner safeguards remain enforced.
- Indexed/member effect-write support from `f16b244` remains active and is consumed normally by beta upvalue recovery/CF.
- Latest fixture checkpoint is `b70bb2d Add repeat-until sample 29`; latest numeric-for CF checkpoint is `5f7c7f4 Recover numeric for loops with branched bodies`; tracked samples 1-29 remain paired.
...
- Sample 29: focused `repeat ... until` regression from `repeat; print(12); until math.random(1,2) == 1`. Required source -> local WeAreDevs Medium -> formatter flow succeeds. Normal resolves 4/4 states in one VM function with 28 reaching definitions and no captures/cells. Beta reports 4 blocks / 23 versioned assignments / 5 preserved final state/return writes / 0 cross-state versions / 0 skipped assignments and prunes 4 physical declarations. Beta-CF currently fails closed on the post-test backedge, as expected because `repeat` structuring is not implemented yet. Important local-compiler quirk: `Compiler:compileStatement(RepeatStatement)` evaluates the condition once before jumping into the body, discards that first result, then evaluates the condition again in the real post-body check. With side-effectful `math.random`, this consumes one extra random value before the first iteration, so same-seed readable-source output can differ from Prometheus output. Raw Medium, formatted, normal, and beta outputs still match exactly across seeds 1-20, proving the deobfuscator preserves actual compiler behavior. The normalized beta CFG is state 1 extra pre-condition -> state 2 body -> state 3 post-test check (`true -> 4`, `false -> 2`) -> state 4 return.
