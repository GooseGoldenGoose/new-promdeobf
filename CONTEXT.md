...
sample 63 continuation: `63.optall2.lua` and `63.optall3.lua` were generated from the prior capped `63.optall.lua`. Pass 2: 1000 rounds, 12 adjacent index-base aliases, 292 adjacent index-key folds, size 329890 -> 305899 bytes. Pass 3: 1000 rounds, 119 adjacent index-key folds, size -> 281361 bytes. A `maxRounds=1` probe on optall3 still finds one global fold, so optall3 exists and reparses but is not a fixed point; the per-edit whole-file reparse/1000-round cap remains a performance limitation on this large sample.
